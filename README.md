...
- `src/` — the source of truth for any part of the game that's been
  ...
  main render loop), and `src/models.js` (every unit/building/prop model
  ...
  the 2D top-down canvas renderer and FPS-mode rendering are still
  unextracted, each a candidate for its own future slice). `src/index.template.html` is the
  same document with
...
`src/` (game data, audio, the build-card UI, save/load, campaigns, abilities, the AI, the core renderer, and the 3D model library, for now; more subsystems will move over time), edit the file
