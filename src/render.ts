...
// ---- roads: 8px-per-tile mask of G.pave, softened, with a medial ridge
// (centre line) and its orientation so the shader can paint asphalt,
// gravel shoulders, edge lines and dashes that follow the road.
...
function glRoadTex() {
  ...
  // chamfer distance to the nearest non-road pixel
  const D = new Float32Array(N), BIG = 1e4, a = 1, b = 1.4142;
  for (let i = 0; i < N; i++) D[i] = bin[i] ? BIG : 0;
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) { const i = y * W + x; if (!D[i]) continue; let v = D[i];
    if (x > 0) v = Math.min(v, D[i - 1] + a); else v = Math.min(v, a);
    if (y > 0) { v = Math.min(v, D[i - W] + a); x > 0 && (v = Math.min(v, D[i - W - 1] + b)); x < W - 1 && (v = Math.min(v, D[i - W + 1] + b)); } else v = Math.min(v, a);
    D[i] = v; }
  for (let y = H - 1; y >= 0; y--) for (let x = W - 1; x >= 0; x--) { const i = y * W + x; if (!D[i]) continue; let v = D[i];
    if (x < W - 1) v = Math.min(v, D[i + 1] + a); else v = Math.min(v, a);
    if (y < H - 1) { v = Math.min(v, D[i + W] + a); x < W - 1 && (v = Math.min(v, D[i + W + 1] + b)); x > 0 && (v = Math.min(v, D[i + W - 1] + b)); } else v = Math.min(v, a);
    D[i] = v; }
  const cv = document.createElement("canvas"); cv.width = W, cv.height = H;
  const cx = cv.getContext("2d")!, im = cx.createImageData(W, H), d = im.data;
  for (let y = 1; y < H - 1; y++) for (let x = 1; x < W - 1; x++) { const i = y * W + x, v = D[i], k = 4 * i;
    d[k] = bin[i] ? 255 : 0, d[k + 3] = 255;
    if (v < 7 || v > 9.3) continue;
    let ridge = !0; for (let dy = -1; dy <= 1 && ridge; dy++) for (let dx = -1; dx <= 1; dx++) if (D[i + dy * W + dx] > v + .01) { ridge = !1; break; }
    if (!ridge) continue;
    const hz = D[i - 1] >= v - .5 && D[i + 1] >= v - .5, vt = D[i - W] >= v - .5 && D[i + W] >= v - .5;
    d[k + 1] = 255, d[k + 2] = hz && !vt ? 255 : vt && !hz ? 0 : 128; }
  cx.putImageData(im, 0, 0);
  // soften the mask; keep the centre line crisp but a couple of px wide
  ...
  const bl = oc.getImageData(0, 0, W, H), bd = bl.data;
  for (let y = 1; y < H - 1; y++) for (let x = 1; x < W - 1; x++) { const k = 4 * (y * W + x); let g = 0, o = -1;
    for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) { const q = 4 * ((y + dy) * W + x + dx); if (q < 0 || q >= d.length) continue; d[q + 1] > g && (g = d[q + 1], o = d[q + 2]); }
    bd[k + 1] = d[4 * (y * W + x) + 1] ? 255 : (d[4 * (y * W + x - 1) + 1] || d[4 * (y * W + x + 1) + 1] || d[4 * ((y - 1) * W + x) + 1] || d[4 * ((y + 1) * W + x) + 1]) ? 110 : 0, bd[k + 2] = o < 0 ? 128 : o; }
  oc.putImageData(bl, 0, 0);
  ...
  m.onBeforeCompile = (sh: any, r: any) => {
    ...
    sh.fragmentShader = "uniform sampler2D tDetail;\nuniform sampler2D tRoad;\nvarying vec3 vTN;\n" + sh.fragmentShader.replace("#include <color_fragment>", `#include <color_fragment>
      { vec2 wp = vWorldFog.xz;
        ...
        #define CH_SEL .b
        float bandA = TRI(1100.0, 260.0, ch), bandB = TRI(1100.0, 260.0, ch + 3.5), bandC = TRI(700.0, 110.0, ch * 1.07 + 9.0);
        #undef CH_SEL
        #define CH_SEL .r
        float fiss = TRI(120.0, 620.0, ch), grit = TRI(40.0, 40.0, ch);
        #undef CH_SEL
        #undef TRI
        float strat = mix(bandA, bandC, 0.4);
        float ledge = clamp((bandA - bandB) * 3.2, -1.0, 1.0);
        vec3 rockLo = vec3(0.27, 0.22, 0.17), rockHi = vec3(0.46, 0.39, 0.3), rockDk = vec3(0.16, 0.13, 0.1);
        ...
        // roads: asphalt with patches and cracks, gravel shoulders, faded
        // edge lines and a dashed centre line following the road.
        ...
        if (rd.r > 0.1) {
          ...
          float edgeL = smoothstep(0.6, 0.66, rd.r) * smoothstep(0.78, 0.72, rd.r) * step(0.35, tdD2);
          asph = mix(asph, vec3(0.62, 0.6, 0.55), edgeL * 0.4);
          float along = rd.b > 0.75 ? wp.x : rd.b < 0.25 ? wp.y : (wp.x + wp.y) * 0.7071;
          float dash = step(fract(along / 34.0), 0.45) * smoothstep(0.5, 0.95, rd.g);
          asph = mix(asph, vec3(0.78, 0.66, 0.26), dash * 0.7 * (0.6 + 0.4 * tdG));
