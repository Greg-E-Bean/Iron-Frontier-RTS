...
e.push(P_(BOXM(24,1.6,.6,.3),2,0,22,"darkmetal"));
e.push(P_(CYL(1.2,5,8),-28,0,13,org?"psi":"gunmetal",{ty:-PI2,e:org}));
e.push(P_(BOXM(9,1.2,.5,.3),-28,0,17,"darkmetal"));
...
break;}
case"factory":{
const sw=.32*r,sd=.28*r,sh=.34*s;
n.push(P_(SLAB(roundRectProfile(.9*r,.9*r,3,3),1.1,.5,"fcpad"),0,0,l,"asphalt"));
if(a){
n.push(P_(BOXM(sw,sd,sh,.25),.3*-r,.3*-r,l,"armor"));
n.push(P_(BOXM(sw*1.04,sd*1.04,1.1,.2),.3*-r,.3*-r,l+sh,"armor3"));
windows(n,sw*.9,sd*.85,l+.3*sh,.22*sh,3);
for(let e=0;e<2;e++)fan(n,.3*-r-.08*sw+e*sw*.45,.3*-r-.12*sd,l+sh,3.5,4,"gunmetal",2.5);
n.push(P_(BOXM(.1*r,.08*r,.25,.12),.3*-r,.3*-r+.12*sd,l+sh,"darkmetal"));
n.push(P_(BOXM(.09*r,.07*r,.2,.1),.3*-r,.3*-r+.12*sd,l+sh+.25,"crystal",{e:1,ty:.25}));
}else if(o){
n.push(P_(BOXM(sw,sd,sh,.3),.3*-r,.3*-r,l,"concrete"));
n.push(P_(BOXM(sw*1.05,sd*1.05,1.3,.25),.3*-r,.3*-r,l+sh,"concrete2"));
windows(n,sw*.9,sd*.85,l+.28*sh,.2*sh,3,"glassdark");
for(const e of[-1,1])stack(n,.3*-r+e*sw*.3,.3*-r-.1*sd,l+sh,.03*r,.12*r,"rust");
}else{
hive(n,{x:.3*-r,y:.3*-r,z:l,r:.14*r,h:sh,crown:.056*r,seg:8,pods:3,crest:!1,vein:"psi"});
n.push(P_(CYL(1.8,2.4,10),.3*-r,.3*-r,l+.2*r,"psi",{e:1,a:{spin:1.2}}));
}
gantry(n,.18*r,.12*r,l,.38*r,.22*r,0);
for(let e=0;e<4;e++)n.push(P_(CYL(1.6,2.5,12),.05*r+e*r*.09,.28*r,l+1.5,"steel",{a:{spin:1.5+.15*e}}));
n.push(P_(CYL(1.2,1.5,10),.18*r,.28*r,l+3.8,"glow",{e:1,a:{spin:2}}));
hazard(n,.2*r,.08*r,4.2,.35*r,0);
crate(n,-.3*r,.26*r,l,5,"olive",-.2);
containerBox(n,-.32*r,-.12*r,l,5.5,2.8,2.5,"rust",.25);
barrel(n,.32*r,-.28*r,l);
break;}
