...
function fpsShoot(e){if(e.cool>0)return;const vm=FPS.vm,magSize=vm&&MAG_SIZE[vm.userData.magFamily];if(magSize&&vm.userData.reloadT>0)return;if(magSize&&null==vm.userData.mag)vm.userData.mag=magSize;const t=e.d,n=Math.cos(FPS.yaw),a=Math.sin(FPS.yaw),r=1.05*(t.range||120)*(FPS.aiming?1.2:1),o=fpsAimTarget(e);FPS.viewKick=Math.min(.09,(FPS.viewKick||0)+.028),o&&(FPS.hitFlashT=.15),o?(e.cool=t.rof*vRof(e),"mind"!==t.role?fire(e,o,t):mindTick(e,o)):"mind"!==t.role&&t.dmg>0&&(e.cool=t.rof*vRof(e),fire(e,{x:e.x+n*r,y:e.y+a*r,e:"u",d:{armor:"veh"},dead:!1} as any,t));magSize&&(spawnShell(vm),vm.userData.mag--,vm.userData.mag<=0&&(vm.userData.reloadT=RELOAD_DUR,vm.userData.mag=0))}
...
return flash.visible=!1,g.userData.flash=flash,g.userData.magFamily="pistol",addMuzzleLight(flash),g.visible=!1,g}
if("tool"===kind){
...
function spawnShell(vm){const flash=vm.userData.flash;if(!flash)return;const m=new THREE.Mesh(new THREE.CylinderGeometry(.006,.006,.018,6),new THREE.MeshStandardMaterial({color:14071847,roughness:.3,metalness:.8}));m.position.set(flash.position.x+.03,flash.position.y-.01,flash.position.z+.15),m.userData.t0=S.time,m.userData.dx=.22+.18*Math.random(),m.userData.dy=.3+.2*Math.random(),m.userData.dz=.12*(Math.random()-.5),vm.add(m),(vm.userData.shells=vm.userData.shells||[]).push(m)}
function tickShells(vm){const shells=vm.userData.shells;if(!shells||!shells.length)return;const flash=vm.userData.flash,bx=flash?flash.position.x+.03:0,by=flash?flash.position.y-.01:0,bz=flash?flash.position.z+.15:0;for(let i=shells.length-1;i>=0;i--){const s=shells[i],dt=S.time-s.userData.t0;if(dt>.45){vm.remove(s),shells.splice(i,1);continue}s.position.set(bx+s.userData.dx*dt,by+s.userData.dy*dt-2.4*dt*dt,bz+s.userData.dz*dt),s.rotation.x=16*dt,s.rotation.z=11*dt}}
...
function furnishInterior(g,bld,half,floorZ,tall){
  ...
  if("civ3"===key){
    ...
    if("hq"===theme){
      ...
      cyl(11,rh-8,0,0,floorZ,3355443,im.trim);
      cyl(7,rh-4,0,0,floorZ,16118491,emisMat(16118491));
