...
function syncBombProp(bld){const mesh=bld.bombMesh;if(!mesh)return;const p=1-Math.max(0,bld.bombT||0)/Math.max(.01,bld.bombFuse||8),hz=2+10*p,pulse=.5+.5*Math.sin(S.time*6.283*hz);mesh.userData.blink.material.color.setRGB(1,.15+.1*pulse,.1),mesh.userData.light.intensity=.6+1.4*pulse}function fpsAimTarget(e){const t=e.d,w=heroWpn(e),aim=FPS.aiming,r=1.05*(w.range||120)*(aim?1.2:1),coneMin=(aim?.975:.945)-(FPS.crouching?.02:0),n=Math.cos(FPS.yaw),a=Math.sin(FPS.yaw);let o=null,s=-1;const i=arr=>{for(const l of arr){if(l.dead||l.owner===e.owner||void 0===l.owner)continue;if("u"===l.e&&l.inside&&l.inside!==e.inside)continue;const p="u"===l.e&&l.inside?interiorFigurePos(l):{x:l.x,y:l.y};const dx=p.x-e.x,dy=p.y-e.y,d=Math.hypot(dx,dy);if(d>r||d<1)continue;if("u"===l.e&&(l.alt||0)>2&&!t.aa&&!l.inside)continue;if("b"===l.e&&t.noBld)continue;const f=(dx*n+dy*a)/d;if(f<coneMin)continue;if(!e.inside&&!((l.alt||0)>2)&&!hasLineOfFire(e.x,e.y,p.x,p.y,heightAt(e.x,e.y)+losEyeH(e),heightAt(p.x,p.y)+losEyeH(l)))continue;const h=1e3*f-.05*d;h>s&&(s=h,o=l)}};return i(S.units),i(S.blds),o}
...
function furnishInterior(g,bld,half,floorZ,tall){
  ...
  if("civ3"===key){
    ...
    box(half*2-16,7,3,0,-half+10,floorZ,8016432,im.trim);
    box(6,4,2,-half+20,-half+10,floorZ+3,7031343,im.trim),cyl(1.6,3,-half+20,-half+10,floorZ+5,12092939,metalMat(12092939));
    for(const[px,pz]of[[-half+12,-half+22],[-half+12,-half+34],[half-12,-half+22]])box(9,9,9,px,pz,floorZ,8016432,im.trim);
    for(const[px,pz]of[[half-13,-half+34],[half-13,-half+46]])cyl(6,14,px,pz,floorZ,7029795,im.trim);
    for(const[cx,cc]of[[-half+14,4165439],[-half+22,11680314],[-half+30,14845739]])sph(3,cx,half-16,floorZ,cc);
    box(1,1,22,0,-half+14,floorZ+26,4737096,metalMat(4737096));
    for(const cz of[-half+11,-half+14,-half+17])sph(1.6,0,cz,floorZ+6,14206090);
  }else if("civ8"===key){
    box(half*2-20,6,15,0,-half+9,floorZ,5912608,im.trim),box(half*2-24,3,20,0,-half+7,floorZ+15,4007959,im.trim);
    for(const[bx,bc]of[[-half+10,3038271],[-half+2,6044186],[half*.1,3038271],[half*.2,6044186],[half*.3,3038271]])box(2,2,5,bx,-half+7,floorZ+18,bc);
    for(const[cx,cz]of[[-half*.56,-half*.25],[half*.44,-half*.31]]){cyl(8,3,cx,cz,floorZ,8016432,im.trim);for(const[sx,sz]of[[cx-8,cz],[cx+8,cz],[cx,cz-8]])cyl(2.4,7,sx,sz,floorZ,7031343,im.trim)}
    for(const[px,pz]of[[half-12,half-12],[half-12,half-22]])cyl(6,14,px,pz,floorZ,7029795,im.trim);
    box(16,10,3,-half+16,half-14,floorZ,9079430,im.trim),box(9,5,10,-half+16,half-14,floorZ+3,2829099),sph(3,-half+16,half-14,floorZ+8,16742953,emisMat(16742953));
  }else if(tall){
