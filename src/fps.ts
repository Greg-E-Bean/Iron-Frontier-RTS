...
const VM_KEY={marksman:"sniper",engineer:"tool",phaser:"beam_temporal",vindicator:"beam_plasma",leech:"beam_drain"};
...
function furnishTheme(theme,c){
  ...
  const acc=fac==="legion"?16734780:fac==="syndicate"?13208831:9427199,base=fac==="legion"?8018490:fac==="syndicate"?6969984:10134445,dark=fac==="legion"?3814440:fac==="syndicate"?3549768:3159597;
  ...
  const beacon=(x,y,z)=>{cyl(1.4,2,x,z,y,dark,metalMat(dark));c.cols.pop();const b=add(new T3.BoxGeometry(3.4,1.6,1),emisMat(fac==="syndicate"?acc:16753920),x,y+3,z);anims.push(t=>{b.rotation.y=5*t})};
  const crate=(x,z,s,y?)=>box(s,s,s,x,z,y,fac==="syndicate"?6969984:fac==="legion"?6309413:7361622,im.trim);
  ...
  if(theme==="assembly"){
    ...
    box(26,18,7,0,-6,F,dark,im.trim);const map=add(new T3.PlaneGeometry(24,16),emisMat(fac==="syndicate"?acc:4251856),0,F+7.3,-6);map.rotation.x=-Math.PI/2;
    ...
    box(16,10,6,0,-4,F,fac==="legion"?6309413:7361622,im.trim);const mp=add(new T3.PlaneGeometry(14,8),propMat(12893352),0,F+6.2,-4);mp.rotation.x=-Math.PI/2;
    const dm=grp(H-12,F,H-18);add(new T3.CylinderGeometry(.6,.6,26,6),metalMat(dark),0,13,0,dm);const bag=new T3.Group;bag.position.y=24,dm.add(bag);add(new T3.CylinderGeometry(3,3,11,10),propMat(fac==="legion"?9127187:5263440),0,-9,0,bag);c.cols.push({x:H-12,z:H-18,r:4});
    anims.push(t=>{bag.rotation.z=.15*Math.sin(2.2*t),bag.rotation.x=.08*Math.sin(1.3*t)});
    crate(-H+8,-H+14,6),crate(-H+8,-H+22,5);
    if(fac==="legion")for(const sx of[-1,1]){const bn=add(new T3.PlaneGeometry(10,18),emisMat(13122091),sx*14,F+rh-20,-H+.7);bn.material.emissiveIntensity=.25}
  }else if(theme==="reactor"){
    decal(40,40,0,0,dark);
    const coreCol=fac==="syndicate"?acc:fac==="legion"?9427199:6281471,core=cyl(5,rh-4,0,-6,F,coreCol,emisMat(coreCol));c.cols.pop();c.cols.push({x:0,z:-6,r:11});
    const cage=[];for(let i=0;i<8;i++){const a=i*.7854;cyl(.8,rh-6,8*Math.cos(a),-6+8*Math.sin(a),F,3355443,metalMat(3355443));c.cols.pop()}
    const ring=add(new T3.TorusGeometry(9,.7,8,32),metalMat(fac==="legion"?9127187:5921370),0,F+rh-10,-6),ring2=add(new T3.TorusGeometry(9.5,.45,8,32),emisMat(coreCol),0,F+rh/2,-6);ring.rotation.x=Math.PI/2;
    const arcs=[];if(fac==="legion")for(let i=0;i<4;i++){const a=add(new T3.CylinderGeometry(.25,.25,10,4),emisMat(12640255),0,F+rh/2,0);arcs.push(a)}
    ...
    const oreCol=fac==="syndicate"?acc:14064174,ores=[];for(let i=0;i<9;i++)ores.push(add(new T3.DodecahedronGeometry(1.5+.6*(i%3),0),new T3.MeshStandardMaterial({color:oreCol,emissive:oreCol,emissiveIntensity:.25,roughness:.5}),0,7,0,belt));
...
function furnishInterior(g,bld,half,floorZ,tall){
  ...
  if("civ3"===key){
    ...
    furnishTheme(PROD_THEME[key],{g,half,floorZ,rh:tall?ROOM_H:ROOM_H_SHORT,box,cyl,sph,emisMat,metalMat,propMat,im,fac:S.players[bld.owner]&&S.players[bld.owner].fac||"vanguard",anims:g.userData.anims||(g.userData.anims=[]),cols});
  }else{
