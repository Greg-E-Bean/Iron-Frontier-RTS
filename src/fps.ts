export {};
function projModel(kind,fac){const e=[],glowC="allied"===fac?"#8fe0ff":"soviet"===fac?"#ff9c4a":"yuri"===fac?"#c98cff":"glow",bodyC="soviet"===fac?"#8a7a6a":"allied"===fac?"#d8e8ee":"yuri"===fac?"#7a5f8a":"white",noseC="soviet"===fac?"#c0392b":"allied"===fac?"#3ba0c9":"yuri"===fac?"#8c3fc9":"red",shellC="soviet"===fac?"#e8c04a":"allied"===fac?"#c9d8e0":"yuri"===fac?"#b98cd8":"gold";return"bullet"===kind||"flak"===kind?e.push(P_(CYL(.5,4.5,6),0,0,0,glowC,{ty:PI2,e:1})):"flame"===kind?e.push(P_(CYL(.9,3,6),0,0,0,"yuri"===fac?"#8fe06a":"tibGlit",{ty:PI2,e:1})):"rocket"===kind||"missile"===kind||"aamissile"===kind?(e.push(P_(CYL(.7,4.5,7),0,0,0,bodyC,{ty:PI2})),e.push(P_(CONE(.7,.2,1.6,7),4.5,0,0,noseC,{ty:PI2})),e.push(P_(CYL(.95,1.6,6),-.6,0,0,glowC,{ty:PI2,e:1}))):"bomb"===kind?e.push(P_(CYL(1.1,3.5,8),0,0,0,"darkmetal",{ty:PI2})):"grenade"===kind?(e.push(P_(CYL(1.3,1.9,8),0,0,0,"olive",{ty:PI2})),e.push(P_(CYL(.35,.5,6),0,1.1,0,"darkmetal",{ty:PI2}))):e.push(P_(CYL(.55,3.4,6),0,0,0,shellC,{ty:PI2})),e}const FPS={on:!1,u:null,yaw:0,pitch:-.03,cam:null,mv:{f:0,s:0},firing:!1,aiming:!1,look:null,stick:null,vm:null,vvm:null,interact:null,entering:null,viewKick:0,hitFlashT:0,aimTarget:null,aimLockT:0,thirdPerson:!1,sprinting:!1,stamina:1,crouching:!1,lastStepT:0,damageFlashT:0,damageDir:0,killFlashT:0,lastEngineSfxT:0,deathT:0,jumpT:0,jumpZ:0,chargingGrenade:!1,swimming:!1,nextAmbientT:0};let fpsPromptTarget=null;let rotateTarget=null;function fpsEyeH(e){return("inf"===e.d.kind?19:"kirov"===e.key?44:e.d.fly?22:34)-(FPS.crouching&&"inf"===e.d.kind?6:0)}function enterFPS(e){return!(!GL||!e||e.dead)&&(FPS.cam||(FPS.cam=new THREE.PerspectiveCamera(74,CW/CH,.6,5200),FPS.cam.up.set(0,1,0),GL.scene.add(FPS.cam)),FPS.on=!0,FPS.u=e,FPS.yaw=e.ang,FPS.pitch=-.04,FPS.mv.f=0,FPS.mv.s=0,FPS.firing=!1,FPS.aiming=!1,FPS.interact=null,FPS.sprinting=!1,FPS.stamina=1,FPS.crouching=!1,FPS.lastStepT=e.animT||0,FPS.lastEngineSfxT=e.animT||0,FPS.damageFlashT=0,FPS.killFlashT=0,FPS.deathT=0,FPS.jumpT=0,FPS.jumpZ=0,FPS.chargingGrenade=!1,FPS.swimming=!1,FPS.nextAmbientT=S.time+rnd(4,10),startFpsAmbience(),e.fps=1,e.order="idle",e.path=null,e.target=null,document.body.classList.add("fps"),document.getElementById("fpsui").classList.remove("hidden"),S.sel=[e],S.autopilot=autopilotEnabled&&0===e.owner&&!(S.mission&&S.mission.hero)&&S.blds.some(b=>b.owner===0&&!b.dead),S.autopilot&&(S.players[0].ai.mimicProfile=inferPlayerProfile(S.players[0])),!0)}function exitFPS(){stopFpsAmbience(),FPS.cam&&FPS.cam.up.set(0,1,0),grenadeArc&&(grenadeArc.visible=!1),FPS.chargingGrenade=!1,FPS.u&&(FPS.u.fps=0,FPS.u.order="idle",FPS.u.path=null),FPS.on=!1,FPS.u=null,FPS.firing=!1,FPS.aiming=!1,FPS.interact=null,FPS.deathT=0,S.autopilot=!1,S.players[0]&&(S.players[0].ai.mimicProfile=null),document.exitPointerLock&&document.pointerLockElement&&document.exitPointerLock(),document.body.classList.remove("fps"),document.getElementById("fpsui").classList.add("hidden")}
let grenadeArc=null;
...
function syncBombProp(bld){const mesh=bld.bombMesh;if(!mesh)return;const p=1-Math.max(0,bld.bombT||0)/Math.max(.01,bld.bombFuse||8),hz=2+10*p,pulse=.5+.5*Math.sin(S.time*6.283*hz);mesh.userData.blink.material.color.setRGB(1,.15+.1*pulse,.1),mesh.userData.light.intensity=.6+1.4*pulse}function fpsAimTarget(e){const t=e.d,aim=FPS.aiming,r=1.05*(t.range||120)*(aim?1.2:1),coneMin=(aim?.975:.945)-(FPS.crouching?.02:0),n=Math.cos(FPS.yaw),a=Math.sin(FPS.yaw);let o=null,s=-1;const i=arr=>{for(const l of arr){if(l.dead||l.owner===e.owner||void 0===l.owner)continue;if("u"===l.e&&l.inside&&l.inside!==e.inside)continue;const p="u"===l.e&&l.inside?interiorFigurePos(l):{x:l.x,y:l.y};const dx=p.x-e.x,dy=p.y-e.y,d=Math.hypot(dx,dy);if(d>r||d<1)continue;if("u"===l.e&&(l.alt||0)>2&&!t.aa)continue;if("b"===l.e&&t.noBld)continue;const f=(dx*n+dy*a)/d;if(f<coneMin)continue;if(!e.inside&&!((l.alt||0)>2)&&!hasLineOfFire(e.x,e.y,p.x,p.y,heightAt(e.x,e.y)+losEyeH(e),heightAt(p.x,p.y)+losEyeH(l)))continue;const h=1e3*f-.05*d;h>s&&(s=h,o=l)}};return i(S.units),i(S.blds),o}
...
function fpsShoot(e){if(e.cool>0)return;const vm=FPS.vm,magSize=vm&&MAG_SIZE[vm.userData.magFamily];if(magSize&&vm.userData.reloadT>0)return;if(magSize&&null==vm.userData.mag)vm.userData.mag=magSize;const t=e.d,n=Math.cos(FPS.yaw),a=Math.sin(FPS.yaw),r=1.05*(t.range||120)*(FPS.aiming?1.2:1),o=fpsAimTarget(e);FPS.viewKick=Math.min(.09,(FPS.viewKick||0)+.028),o?(e.cool=t.rof*vRof(e),"mind"!==t.role?fire(e,o,t):mindTick(e,o)):"mind"!==t.role&&t.dmg>0&&(e.cool=t.rof*vRof(e),fire(e,{x:e.x+n*r,y:e.y+a*r,e:"u",d:{armor:"veh"},dead:!1} as any,t));magSize&&(vm.userData.akimbo&&(vm.userData.altFire=!vm.userData.altFire),spawnShell(vm),vm.userData.mag--,vm.userData.mag<=0&&(vm.userData.reloadT=RELOAD_DUR,vm.userData.mag=0))}
...
function fpsAbility(){const e=FPS.u,info=fpsAbilityInfo(e);if(!info)return void hint("No special ability for this unit");if(e.abilityCD>0)return;const kind=e.d.proj;"bullet"===kind?throwGrenade(e):"flame"===kind?flameNova(e):("missile"===kind||"rocket"===kind)&&e.d.deploy&&toggleDeploy(e),e.abilityCD=info.cd}
...
const VM_KEY={marksman:"sniper",tanya:"pistol",reaper:"pistol",phantom:"pistol",engineer:"tool",chrono:"beam_temporal",vindicator:"beam_plasma",leech:"beam_drain"};
...
Object.assign(window, {
  ...
  fpsAbility, fpsAbilityDown, fpsAbilityUp, fpsJump, fpsPassable, fpsDeathTick,
