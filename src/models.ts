...
grp=(i+(side>0?0:1))%2,c=(((a||0)&3)+2*grp)&3,lift=1===c?2.8:0,sw=[-1,0,1,0][c]*.08*t,
kx=hx+side*1.5+.5*sw,ky=side*(.5*r+4),kz=.46*hipZ+lift,
fx=hx+side*2.4+sw,fy=side*(.5*r+6-.6*lift),fz=1.4+.55*lift;
...
function droneModel(e){const t=[];t.push(P_(DOME(3.4,2.6,10),0,0,3.4,"carapace2")),t.push(P_(DOME(1.8,1.4,8),0,0,5.6,"carapace")),t.push(P_(DOME(.9,.7,8),0,0,6.9,"psi",{e:1}));for(let i=0;i<4;i++){const a=i*PI2+.6;t.push(P_(CYL(.35,3.6,6),3.1*Math.cos(a),3.1*Math.sin(a),2.4,"carapace2",{r:a,ty:.6})),t.push(P_(DOME(.5,.4,6),4.8*Math.cos(a),4.8*Math.sin(a),.6,"carapace",{e:0}))}return t}function bastionModel(e,fac){const t=[],r=40,n=16,hz=5,hh=6.5,top=hz+hh;trackUnit(t,r,n,9,e,{trkW:8,wheels:6});t.push(P_(SLAB(hullProfile(.92*r,n+2,fac),hh,1.8,"bstH"+(fac||"")),0,0,hz,"armor2"));
for(const sx of[-1,1])for(const sy of[-1,1]){strut3(t,sx*.4*r,sy*.3*n,top-1.2,sx*.56*r,sy*.36*n,2.6,1.05,"gunmetal");t.push(P_(CYL(.9,1.4,8),sx*.4*r,sy*.3*n,top-1.8,"darkmetal"));t.push(P_(CYL(2.2,1,10),sx*.56*r,sy*.36*n,1.6,"darkmetal"))}
if("soviet"===fac){
...
t.push(P_(DOME(3.4,3.2,12),.22*r,0,top+2.2,"psi",{e:1,a:{spin:.6}}));t.push(P_(CYL(3.6,1.2,12),.22*r,0,top+1.4,"dark2"));
for(const sy of[-1,1]){const bx=.1*r,by=sy*.26*n,bz=top+3;strut3(t,bx,by,bz,.34*r,sy*.42*n,top-1,.9,"carapace");strut3(t,.34*r,sy*.42*n,top-1,.5*r,sy*.3*n,top-3,.6,"dark2");t.push(P_(DOME(.9,.8,7),.5*r,sy*.3*n,top-3.4,"psi",{e:1}));t.push(P_(DOME(2.4,2,9),-.4*r,sy*.2*n,top,"bile",{e:1}))}
}else{
