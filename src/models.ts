...
// Construction reveal stage (0=bare pad+crane, 4=fully built). Only "conyard"
// actually varies this - render.ts passes the live rise fraction (0-1) via
// dep for that key only, and bakes the resulting bucket into its geometry
// cache key (like gateBucket does for gates) so each stage gets its own
// cached mesh. Every other building key gets bucket 4 (always fully built),
// so this is a no-op for them.
const riseBucket="conyard"===e?Math.round(4*Math.max(0,Math.min(1,+dep||0))):4;
