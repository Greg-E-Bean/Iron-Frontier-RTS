...
// "hive" is deliberately excluded: it already uses the dep parameter for
// its own deploy-state toggle (see the "hive" case below), which gates
// real gameplay (tickHive's drone spawning in src/sim.ts) - routing the
// rise fraction through dep for it too would silently break that toggle.
function hasStagedBuild(e){return"conyard"===e||"factory"===e||"power"===e||"barracks"===e||"refinery"===e||"airfield"===e||"navalyard"===e}
...
if(riseBucket>=1){
n.push(P_(BOXM(.46*r,.4*r,.32*r,1.2),-.14*r,-.16*r,l,a?"armor":o?"concrete":"carapace"));
}
if(riseBucket>=2){
n.push(P_(BOXM(.38*r,.32*r,.06*r,1),-.14*r,-.16*r,l+.32*r,a?"armor2":o?"concrete2":"carapace2"));
n.push(P_(BOXM(.4*r,.02*r,.16*r,.1),-.14*r,-.37*r,l+.1*r,a?"glass":o?"glassdark":"psi",{e:1}));
}
if(riseBucket>=3){
gantry(n,.2*r,.05*r,l,.4*r,.24*r,1.5708);
}
if(riseBucket>=4){
n.push(P_(BOXM(.1*r,.1*r,.6*r,.3),.26*r,.18*r,l,"gunmetal"));
n.push(P_(BOXM(.5*r,.09*r,.09*r,.5),.02*r,.18*r,l+.58*r,"steel"));
n.push(P_(CYL(1.5,1.6,10),.24*r,.18*r,l+.6*r,a?"crystal":o?"glow":"psi",{e:1,a:{spin:a?1.4:0}}));
if(a)dish(n,-.14*r,-.42*r,l+.32*r+.06*r,.045*r);
else if(o)stack(n,-.3*r,-.3*r,l,.025*r,.14*r,"rust");
else n.push(P_(DOME(.05*r,.045*r,10),-.14*r,-.42*r,l+.32*r+.06*r,"psi",{e:1,a:{spin:.6}}));
}
