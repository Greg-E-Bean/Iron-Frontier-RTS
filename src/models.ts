...
// Road bridge deck for one tile. v: 0 single-lane span (both rails, centre
// dashes down the middle); 1/2 = near/far half of a two-tile road bridge,
// railed on the outer side with the dashed centre line on the inner edge.
const e=[],L=32.3,side=v===1||v===2?v:0,sg=1===side?-1:1,ASPH="#34353a",LINE="#d8d4c6",DASH="#d9b23e";
...
for(const o of outs){const yo=side?yOut:o*20;
 ...
 e.push(P_(BOXM(L,.55,.05),0,yo-o*4,1.72,LINE));
...
const dy=side?yIn:0,dw=side?.7:1.3;for(const dx of[-8,8])e.push(P_(BOXM(9,dw,.05),dx,dy+(side?sg*dw/2:0),1.72,DASH));
