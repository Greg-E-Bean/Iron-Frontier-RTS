...
walls.forEach(([wx,wy,wh,a],k)=>{const c=Math.cos(a),sn=Math.sin(a),ox=wx-c*.8,oy=wy-sn*.8,ex=wx+c*.34*r,ey=wy+sn*.34*r;const el=Math.min(.64*r,Math.hypot(ex,ey));const fx=Math.cos(a+.12)*el,fy=Math.sin(a+.12)*el;taper3(out,ox,oy,4.6,fx,fy,.4,1.8,.35,"flesh");taper3(out,ox,oy,5.2,fx,fy,.9,.22,.12,"psi",{e:1});out.push(P_(DOME(2,1.3,8),ox,oy,3.8,"carapace2"));
if(k%2)out.push(P_(CONE(1.5,.1,Math.min(.35*wh,14)+3,7),wx-c*.4,wy-sn*.4,4,"bone",{ty:.35,r:a}));else out.push(P_(DOME(2.6,2.2,10),wx+c*.6,wy+sn*.6,3.2,"bile",{e:1,a:{bob:1.3+.23*k,ba:.45}}))});
