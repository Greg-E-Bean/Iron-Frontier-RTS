...
return a}
function syndicateTurret(e,t,r,n){const a=[],o=e+3.6;a.push(P_(CYL(.85*t,2.6,12),0,0,e-2.3,"flesh"));ell(a,0,0,e-.6,1.05*t,.98*t,.62*t,"body");ell(a,-.14*t,0,e+.1,.95*t,1.02*t,.5*t,"carapace");ell(a,-.32*t,0,e+1.2,.7*t,.8*t,.45*t,"carapace2");
...
const PILLAR_H={warden:27,mauler:30,fangtank:26};const TURRET_RISE={warden:15.5,mauler:17.5,fangtank:16.6};
...
"legion"===n.fac?(a.push(P_(CYL(.11*t,3.4,10),-.12*e,0,th+1.8,"rust")),a.push(P_(CYL(.12*t,.6,10),-.12*e,0,th+5.2,"black"))):(a.push(P_(CYL(.5,1.6,6),-.12*e,0,th+1.8,"gunmetal")),a.push(P_(DOME(1.4,1.4,10),-.12*e,0,th+3.4,"white")));}
return a;
}
...
n=n||{};
const a=[],rad=.42*t,org="syndicate"===n.fac,sov="legion"===n.fac,zc=rad,L=e;
...
n.deployed?deployedOutriggers(a,e,t,o):"syndicate"===n.fac?legWalker(a,e,t,o,r,n.legs||6,n.lo):n.wheeled?wheelUnit(a,e,t,o,r):trackUnit(a,e,t,o,r,n);
const s=.52*o,org="syndicate"===n.fac;
...
{const fg=FGLOW[n.fac]||"arcbolt";if(org){a.push(P_(DOME(1.3,1,8),.47*e,0,s+4.2,"psi",{e:1}));for(const sy of[-1,1])a.push(P_(BOXM(.46*e,.3,.35,.1),-.04*e,sy*(.5*t+.95),s+5.2,"psi",{e:1}))}else{for(const sy of[-1,1]){a.push(P_(BOXM(.56*e,.28,.34,.1),-.02*e,sy*(.5*t+.82),s+5.9,fg,{e:1}));a.push(P_(BOXM(.9,.5*.3*t,.6,.1),-.505*e,sy*.36*t,s+4.6,"red",{e:1}));a.push(P_(BOXM(1.1,1.1,.9,.2),.5*e,sy*.3*t,s+1.8,"darkmetal"));if(!n.skirt&&!n.wheeled&&!n.deployed)for(let k=0;k<4;k++)a.push(P_(BOXM(.19*e,.6,2.5,.3),(-.3+.2*k)*e,sy*(.75*t+2.2),s+2.7,k%2?"armor3":"armor2"))}a.push(P_(BOXM(.3,.46*t,.45,.1),.53*e,0,s+4.5,fg,{e:1}));a.push(P_(SLAB(roundRectProfile(.26*e,.56*t,1.2,2),1.3,.5,"ed"+e.toFixed(1)+t.toFixed(1)),-.34*e,0,s+6.5,"armor3"));for(const sy of[-1,1])vent(a,-.34*e,sy*.14*t,s+7.8,.18*e,.18*t,"darkmetal");a.push(P_(CYL(.16,9,5),-.42*e,-.36*t,s+7.8,"dark2"));a.push(P_(DOME(.35,.3,6),-.42*e,-.36*t,s+16.8,"red",{e:1}))}}
return a;
}function turretProfile(e,t,fac){const r=e/2,n=t/2;if("syndicate"===fac){const p=[],segs=7;for(let i=0;i<segs;i++){const ang=i/segs*6.283+.15;p.push([Math.cos(ang)*r,Math.sin(ang)*n])}return p}if("vanguard"===fac){const p=[],segs=18;for(let i=0;i<segs;i++){const ang=i/segs*6.283;p.push([Math.cos(ang)*r*.96,Math.sin(ang)*n*.96])}return p}if("legion"===fac)return roundRectProfile(.92*e,.86*t,.055*Math.min(e,t),1);return hexProfile(e,t)}const FGLOW:Record<string,string>={vanguard:"arcbolt",legion:"glow",syndicate:"psi"};function tankTurret(e,t,r,n){n=n||{};if("syndicate"===n.fac)return syndicateTurret(e,t,r,n);const a=[];a.push(P_(CYL(.95*t,2.4,12),0,0,e-2.2,"dark")),a.push(P_(SLAB(turretProfile(2.1*t,1.75*t,n.fac),6.2,2,"tt"+t.toFixed(1)+(n.fac||"")),.08*-t,0,e,"body")),a.push(P_(WEDGE(.7*t,1.45*t,5.4,2.4),.85*t,0,e,"dark")),a.push(P_(BOXM(3.6,1.15*t,5,1.1),1.15*t,0,e+1.4,"gunmetal"));const o=e+3.6;if(n.twin)for(const e of[-3,3])a.push(P_(CYL(1.5,r,9),1.25*t,e,o,"gunmetal",{ty:PI2})),a.push(P_(CYL(2.1,4,9),1.25*t+r-4,e,o,"steel",{ty:PI2}));else if(n.gat){a.push(P_(CYL(3.4,4.4,10),1.2*t,0,o,"steel",{ty:PI2}));for(let e=0;e<6;e++){const s=e*Math.PI/3+(n.f?.5:0);a.push(P_(CYL(.85,r,6),1.3*t,2.3*Math.cos(s),o+2.3*Math.sin(s),"gunmetal",{ty:PI2}))}}else if(n.beam)a.push(P_(CONE(3.6,2.2,6,8),.9*t,0,o-2.4,"crystal",{ty:PI2})),a.push(P_(CYL(1.6,r,8),1.3*t,0,o,"crystal",{ty:PI2})),a.push(P_(CYL(2,2.4,8),1.3*t+r-2.4,0,o,"crystal",{ty:PI2,e:1}));else if(n.pods)for(const e of[-3.4,3.4])a.push(P_(BOXM(9,3.8,3.4,1),.5*t,e,o-.6,"steel")),a.push(P_(CONE(1.5,.4,2.4,7),.5*t+4.6,e,o+.4,"red",{ty:PI2}));else if(n.flame){a.push(P_(CYL(1.8,r,8),1.2*t,0,o,"rust",{ty:PI2})),a.push(P_(CONE(1.8,.6,4,8),1.2*t+r-4,0,o,"darkmetal",{ty:PI2}));for(const e of[-2.6,2.6])a.push(P_(CYL(1.1,5,8),.7*t,e,o-1,"steel",{ty:PI2}));a.push(P_(CYL(1,1.4,6),1.2*t+r-1,0,o,"glow",{ty:PI2,e:1}))}else if(n.dome){a.push(P_(DOME(1.15*t,.95*t,12),0,0,e+6,"psi")),a.push(P_(DOME(.5*t,.5*t,10),0,0,e+6+.9*t,"glass",{e:1}));for(let r=0;r<4;r++){const n=r*PI2+.78;a.push(P_(CYL(.7,6.5,6),Math.cos(n)*t*.9,Math.sin(n)*t*.9,e+6.2,"crystal",{e:1}))}}else a.push(P_(CYL(1.75,r,10),1.25*t,0,o,"gunmetal",{ty:PI2})),a.push(P_(CYL(2.25,.45*r,10),1.25*t+.18*r,0,o,"armor3",{ty:PI2})),a.push(P_(CYL(2.1,.9,10),1.25*t+.1*r,0,o,"darkmetal",{ty:PI2})),a.push(P_(CYL(2.4,4.6,10),1.25*t+r-4.6,0,o,"steel",{ty:PI2})),a.push(P_(CYL(2.9,2.6,10),1.25*t+r-1.4,0,o,"darkmetal",{ty:PI2})),a.push(P_(BOXM(2.6,7,7,.4),1.25*t+r-.5,0,o,"gunmetal")),a.push(P_(BOXM(2.6,7,7,.4),1.25*t+r-3.4,0,o,"gunmetal"));for(const q of a)((q.ty===PI2&&(q.x||0)>=1.2*t-.01)||(q.x||0)>=1.25*t+.5*r)&&(q.b=1);const fg=FGLOW[n.fac]||"arcbolt";n.dome||n.pods||(a.push(P_(SLAB(turretProfile(1.45*t,1.2*t,n.fac),1.5,.7,"tt2"+t.toFixed(1)+(n.fac||"")),-.22*t,0,e+6.2,"armor2")),a.push(P_(BOXM(1.05*t,.3,.4,.1),-.1*t,-.9*t,e+4.1,fg,{e:1})),a.push(P_(BOXM(1.05*t,.3,.4,.1),-.1*t,.9*t,e+4.1,fg,{e:1})),a.push(P_(BOXM(1.7,2.3,1.5,.4),.48*t,-.46*t,e+6.2,"gunmetal")),a.push(P_(CYL(.62,.5,8),.48*t+.9,-.46*t,e+6.95,fg,{ty:PI2,e:1})),a.push(P_(BOXM(.5*t,1.45*t,.3,.1),-1.38*t,0,e+3.4,"dark2")),a.push(P_(DOME(.42,.36,6),1.1*-t,.62*t,e+21.4,"red",{e:1})));for(const sg of[-1,1])for(let k=0;k<3;k++)a.push(P_(CYL(.42,1.7,6),(.42-.1*k)*t,sg*(.82*t),e+4.6+.6*k,"darkmetal",{ty:1,r:sg*.55}));return n.dome||(a.push(P_(CYL(2.9,2.6,10),.42*-t,0,e+6.2,"body")),a.push(P_(DOME(2.9,1.5,10),.42*-t,0,e+8.8,"trim")),a.push(P_(CYL(.6,5.2,6),.42*-t,.75*t,e+7,"gunmetal",{ty:PI2}))),a.push(P_(BOXM(.55*t,1.35*t,3.6,.9),1.2*-t,0,e+1.2,"armor3")),a.push(P_(CYL(.45,8.5,5),1.15*-t,.5*-t,e+5.5,"gunmetal")),a.push(P_(CYL(.22,16,5),1.1*-t,.62*t,e+5.5,"darkmetal",{tx:.12})),a.push(P_(BOXM(1.6,1.6,.6,.2),.15*-t,.7*t,e+2.4,"darkmetal")),seams(a,.8*t,.7*t,e+.2,"dark"),(n.twin||n.gat||n.beam||n.flame)&&a.forEach(q=>{(q.ty===PI2&&(q.x||0)>=1.2*t-.01&&(q.z||0)>=e+1)&&(q.b=1)}),a}function harvesterLegion(e){const t=[],r=46,n=22;trackUnit(t,r,n,11,e,{trkW:9,wheels:6});t.push(P_(SLAB(hullProfile(42,25,"legion"),7.5,2,"hvS1"),0,0,5.5,"armor2"));const h0=t.length;t.push(P_(TSLAB(roundRectProfile(22,27,4,3),14,.86,"hopS"),-7,0,13,"armor3"));t.push(P_(BOXM(20,1.6,11.6,.5),-7,-13.8,14,"armor"));t.push(P_(BOXM(20,1.6,11.6,.5),-7,13.8,14,"armor"));for(let i=0;i<4;i++){const yy=-17+i*7.4;t.push(P_(BOXM(1.2,26.4,1.2,.2),yy,0,14,"rust"))}t.push(P_(BOXM(12,1.4,2,.4),-7,-14.5,24,"steel"));t.push(P_(BOXM(12,1.4,2,.4),-7,14.5,24,"steel"));t.push(P_(BOXM(2.4,26.8,14.4,.6),-18.6,0,13,"armor3"));if(2&e){const c=4&e?"tibB":"tibG",g=4&e?"tibBlit":"tibGlit";t.push(P_(SLAB(roundRectProfile(19.2,24.6,3,2),3.8,1,"ldS"),-7,0,25,c)),t.push(P_(CONE(5.6,1.2,6,8),-4,0,28.4,c)),t.push(P_(CONE(3.4,.7,4.2,7),-11,3.4,28.4,g,{e:1})),t.push(P_(CONE(3,.6,3.8,7),-10,-4,28.4,g,{e:1}))}if(8&e){const px=-18.6,pz=13,a=-.55,ca=Math.cos(a),sa=Math.sin(a);for(let i=h0;i<t.length;i++){const p=t[i],dx=(p.x||0)-px,dz=(p.z||0)-pz;t[i]=Object.assign({},p,{x:px+dx*ca+dz*sa,z:pz-dx*sa+dz*ca,ty:(p.ty||0)+a})}for(const[x,z,k]of[[-21.5,13,0],[-23,8.5,1],[-24,4,2]])t.push(P_(CONE(2.4-.4*k,.5,3,7),x,k%2?2:-2,z,k%2?"tibGlit":"tibG",{e:1}));t.push(P_(CYL(1.6,5,8),-6,0,9,"steel",{ty:-.3}))}t.push(P_(SLAB(roundRectProfile(11,19,1.6,2),12,1.4,"cabS"),12,0,13,"concrete2"));t.push(P_(BOXM(9.4,17,3.8,.4),12,0,22,"armor"));t.push(P_(BOXM(1.4,15.4,4.6,.4),16.6,0,17,"glassdark"));pipeRun(t,-2,-13.4,10,-13.4,17.6,.6,"rust");pipeRun(t,-2,13.4,10,13.4,17.6,.6,"rust");t.push(P_(CYL(1.4,2,8),10,.42*-n,26,"lightY",{e:1}));t.push(P_(CYL(1.4,2,8),10,.42*n,26,"lightY",{e:1}));t.push(P_(CYL(7.6,29,16),22.4,14.5,7.4,"gunmetal",{tx:PI2}));t.push(P_(CYL(8,2.2,16),22.4,-14.5,7.4,"darkmetal",{tx:PI2}));t.push(P_(CYL(8,2.2,16),22.4,14.5,7.4,"darkmetal",{tx:PI2}));for(let i=0;i<10;i++){const yy=-13+2.9*i;for(const ang of[0,1.57,3.14,4.71])t.push(P_(BOXM(1.6,1.6,4.6,.2),22.4+Math.cos(ang)*7.6,yy,7.4+Math.sin(ang)*7.6,"rust",{ty:ang}))}for(let k=0;k<8;k++){const ang=.393+k*.785;t.push(P_(CYL(.5,29,6),22.4+Math.cos(ang)*7.7,14.5,7.4+Math.sin(ang)*7.7,"darkmetal",{tx:PI2}))}t.push(P_(BOXM(10,3.2,3.2,1),.36*r,-13.6,8.4,"steel"));t.push(P_(BOXM(10,3.2,3.2,1),.36*r,13.6,8.4,"steel"));for(const s of[-8.2,8.2]){t.push(P_(CYL(1.8,11,8),-17,s,25,"darkmetal")),t.push(P_(CYL(2.1,2.6,8),-17,s,36,"rust")),t.push(P_(CYL(1.4,3,6),-17,s,38.6,"black"))}bolts(t,-7,-12.9,11,20,0,7,"steel");bolts(t,-7,12.9,11,20,0,7,"steel");seams(t,18,22,12,"dark");t.push(P_(BOXM(2.2,1.6,.9,.25),-15.8,-6.6,14.6,"darkmetal"));t.push(P_(BOXM(2.2,1.6,.9,.25),-15.8,6.6,14.6,"darkmetal"));return t}function harvesterVanguard(e){const t=[],r=42,n=17;trackUnit(t,r,n,8.5,e,{trkW:6.6,wheels:6});t.push(P_(SLAB(hullProfile(38,19,"vanguard"),6.4,2.4,"hvA1"),0,0,5,"armor2"));t.push(P_(TSLAB(roundRectProfile(17,20,5,4),10,.9,"hopA"),-8,0,11.4,"armor2"));t.push(P_(BOXM(15,1.2,8.6,.4),-8,-10.2,12,"armor"));t.push(P_(BOXM(15,1.2,8.6,.4),-8,10.2,12,"armor"));const ul=8&e;t.push(P_(DOME(8.6,6,16),ul?-10:-8,0,ul?24:20.4,"glass",{e:1,ty:ul?-.5:0}));t.push(P_(CYL(8.6,1.2,16),ul?-10:-8,0,ul?24:20.4,"armor3",{ty:ul?-.5:0}));ul&&(t.push(P_(BOXM(1,2,4.4,.2),-1,-6,20.4,"steel")),t.push(P_(BOXM(1,2,4.4,.2),-1,6,20.4,"steel")),[[-19,15],[-20.5,10.5],[-21.5,6]].forEach(([x,z],k)=>t.push(P_(CONE(1.7-.3*k,.4,2.6,7),x,k%2?1.4:-1.2,z,k%2?"tibGlit":"tibG",{e:1}))));t.push(P_(BOXM(9.6,1,.6,.15),-8,-8.4,20,"arcbolt",{e:1}));t.push(P_(BOXM(9.6,1,.6,.15),-8,8.4,20,"arcbolt",{e:1}));if(2&e){const c=4&e?"tibB":"tibG",g=4&e?"tibBlit":"tibGlit";t.push(P_(CONE(4.4,.9,5,10),-8,0,21.2,c)),t.push(P_(CONE(2.4,.4,3.2,7),-11,2.6,21.2,g,{e:1})),t.push(P_(CONE(2.2,.4,3,7),-5.4,-2.6,21.2,g,{e:1}))}t.push(P_(DOME(5.6,5,16),12,0,13.4,"glass",{e:1}));t.push(P_(CYL(5.6,1.3,16),12,0,13.4,"darkmetal"));t.push(P_(BOXM(30,1,.6,.2),0,-9.4,11.6,"arcbolt",{e:1}));t.push(P_(BOXM(30,1,.6,.2),0,9.4,11.6,"arcbolt",{e:1}));t.push(P_(CYL(1.2,1.6,8),9,.42*-n,13,"lightY",{e:1}));t.push(P_(CYL(1.2,1.6,8),9,.42*n,13,"lightY",{e:1}));for(const s of[-6.4,6.4])t.push(P_(CYL(3.1,15,10),19.4,s,7,"steel",{ty:PI2}));t.push(P_(CYL(1,15,10),19.4,0,10,"gunmetal",{ty:PI2}));t.push(P_(SLAB(roundRectProfile(9,15,3,3),2,.9,"scoopA"),22,0,4,"armor3"));t.push(P_(CYL(.4,8,6),-15,7,20.4,"darkmetal"));t.push(P_(CYL(1,1.2,10),-15,7,28.4,"arcbolt",{e:1}));bolts(t,-8,-9.6,11,15,0,3,"steel");bolts(t,-8,9.6,11,15,0,3,"steel");seams(t,15,17,10.6,"armor3");return t}function harvesterSyndicate(e){const t=[],r=40,n=20;legWalker(t,r,n,9.5,e,8);ell(t,1,0,5.6,18,11.5,2.6,"flesh",{tx:Math.PI});ell(t,1,0,5.6,18.5,12,4.4,"carapace2");for(let k=0;k<4;k++)ell(t,12-8*k,0,6.2,5,11.6-.6*k,5.4-.3*k,"carapace");t.push(P_(DOME(11,8,14),-8,2,13,"carapace"));t.push(P_(DOME(8,6.4,12),-9,-6,11.6,"carapace2"));t.push(P_(DOME(6,5,10),-16,1,10.4,"carapace2"));for(let i=0;i<5;i++){const a=.55*i-1.1;t.push(P_(CONE(1,.15,3.4,6),-8+7*Math.cos(a),2+7*Math.sin(a),16.4,"psi",{e:1,r:a}))}if(2&e){const c=4&e?"tibB":"tibG",g=4&e?"tibBlit":"tibGlit";t.push(P_(CONE(4.6,.9,6,9),-8,2,17,c)),t.push(P_(CONE(2.6,.5,3.6,7),-12,5.4,20,g,{e:1})),t.push(P_(CONE(2.4,.4,3.2,7),-5,-1.4,20,g,{e:1}))}t.push(P_(DOME(6,5.4,14),11,-1,12.6,"carapace"));t.push(P_(DOME(2.4,2,10),13.6,-1,17,"psi",{e:1}));for(const[ey,ez]of[[-3,14],[1,14.8],[-1,13]])t.push(P_(DOME(.9,.8,8),15,ey,ez,"psi",{e:1}));const segs=[[15,7,3.8,2.6],[19.5,5.6,2.6,1.7],[23.5,4.4,1.7,.9]];for(const p of segs)t.push(P_(CONE(p[2],p[3],5,9),p[0],1.2,p[1],"carapace2",{ty:PI2}));t.push(P_(DOME(1.6,1.4,10),27,1.2,3.6,"carapace",{ty:PI2}));t.push(P_(CYL(.4,4,6),27.2,1.2,3.6,"psi",{e:1,ty:PI2}));t.push(P_(CONE(2.4,3.4,5,10),-17.6,1,9,"flesh",{ty:-PI2}));for(let i=0;i<3;i++){const yy=-6+i*6;t.push(P_(DOME(1.4,1.1,8),-3+1.4*(i%2),yy,9.4,"psi",{e:1}))}8&e&&(t.push(P_(DOME(6,4.6,12),-8,2,19.4,"bile",{e:1})),t.push(P_(DOME(3.6,3.4,10),-15,1,14.6,"bile",{e:1})),t.push(P_(DOME(3.2,3,10),-9,-6,17,"bile2",{e:1})),[[-4,-6],[-12,7],[-2,6],[-10,-2]].forEach(([x,y])=>t.push(P_(CONE(1.1,.2,5,7),x,y,20.5,"psi",{e:1}))));return t}function harvesterModel(e,fac){return"legion"===fac?harvesterLegion(e):"syndicate"===fac?harvesterSyndicate(e):harvesterVanguard(e)}
function harvesterAugerModel(){
  ...
  return e;
}const MINE_TOOL={drone:[()=>droneLaserModel(),5.5],miner_vanguard:[()=>harvesterDrumModel(),20]},MINE_DEFAULT=[()=>harvesterAugerModel(),22];
...
t.push(P_(BOXM(5.4,5.4,3.2,.5),-.36*r,.18*n,top+5.6,"armor2"));for(const yy of[-1.2,1.2])t.push(P_(CYL(.8,.9,8),-.36*r+2.7,.18*n+yy,top+7.2,"arcbolt",{e:1,ty:PI2}));
...
if("legion"===fac){trackUnit(t,L,W,12,e,{trkW:10,wheels:8});const hz=6,top=13;t.push(P_(SLAB(hullProfile(.94*L,W+4,"legion"),7,1.8,"mcvSH"),0,0,hz,"armor2"));
...
else if("syndicate"===fac){legWalker(t,.9*L,W,9,e,10);const hz=4.5,top=12;ell(t,0,0,hz+.4,.46*L,.5*W+1,2.6,"flesh",{tx:Math.PI});for(let k=0;k<6;k++)ell(t,(.38-.15*k)*L,0,hz,.09*L,.5*W+1.4,7.4,k%2?"carapace":"carapace2");
...
t.push(P_(BOXM(.46*L,W-3,7,.8),-.02*L,0,top,"armor"));for(const sy of[-1,1])for(let k=0;k<3;k++)t.push(P_(BOXM(6,1,3.4,.3),-.14*L+k*.12*L,sy*(hw-.6),top+1.2,"darkmetal"));t.push(P_(DOME(.12*L,5.5,14),.04*L,0,top+7,"armor3"));t.push(P_(DOME(3.4,3,12),.04*L,0,top+12.2,"glass",{e:1}));t.push(P_(CYL(.5,6,8),.04*L,0,top+14.5,"arcbolt",{e:1}));
t.push(P_(BOXM(.2*L,W-4,6,.6),-.38*L,0,top,"darkmetal"));vent(t,-.38*L,4,top+6,8,6,"steel");for(const sy of[-1,1])t.push(P_(CYL(1.1,7,10),-.46*L,sy*6.5,top,"gunmetal")),t.push(P_(CYL(1.3,1,10),-.46*L,sy*6.5,top+7,"black"));for(const cx of[-.34*L,-.15*L])t.push(P_(BOXM(2,6,4.5,.3),cx,0,top+6.5,"armor3"));t.push(P_(CYL(3,.4*L,12),-.08*L,0,top+12.6,"armor2",{ty:-PI2}));t.push(P_(CONE(3,.6,6,12),-.48*L,0,top+12.6,"armor2",{ty:-PI2}));t.push(P_(CONE(1.2,.1,3,8),-.48*L-6,0,top+12.6,"arcbolt",{e:1,ty:-PI2}));
...
function hornetModel(lit){const n=[],z=3.4;jetFuse(n,[[-14,1.6],[-10,2.4],[0,2.7],[7,2.4],[11,1.6],[14,.3]],z,"body","dark");n.push(P_(DOME(2,1.9,12),6.4,0,z+1.7,"glassdark",{e:1,sx:1.9}));n.push(P_(BOXM(4,.3,.3,.05),6.4,0,z+3.6,"trim"));jetWing(n,[[3,2],[-4,12],[-8,12],[-8.5,2]],z+1.2,.8,"body","hrnW");jetWing(n,[[-10,1.5],[-12.5,6],[-14.4,6],[-14.4,1.5]],z-.2,.5,"body","hrnT");jetFin(n,-12.5,0,z+1.8,6,6,1.4,0,"body");for(const sy of[-1,1]){n.push(P_(CYL(1.5,8,12),-6,sy*5.8,z+.2,"gunmetal",{ty:PI2}));n.push(P_(CONE(1.5,.9,2,12),2,sy*5.8,z+.2,"armor3",{ty:PI2}));jetNozzle(n,-6,sy*5.8,z+.2,1.25,lit,"arcbolt");for(const bx of[-1.5,-6.2])jetStore(n,bx-1.5,sy*9.5,z-.8,4.2,.95,"darkmetal","gunmetal");n.push(P_(BOXM(3.6,1.4,2,.3),1.6,sy*2.6,z-1.6,"dark"))}n.push(P_(BOXM(9,2.6,.4,.1),-2,0,z-2.8,"darkmetal"));n.push(P_(CYL(.45,.5,8),-4,0,z-3.1,"lightY",{e:1}));jetGear(n,lit,8,-5.5,5.8,1.2,2.1);return n}
...
function titanLegsMech(e,len,wid,standH,frame,legMat,jointMat,footMat,glow?,armMat?){glow=glow||"arcbolt",armMat=armMat||"body";const hipZ=standH-.6,spanX=.3*len,hipY=.5*wid+.4,L=(a,b,t)=>a+(b-a)*t;for(const side of[-1,1])for(const fb of[-1,1]){const hx=fb*spanX,hy=side*hipY,phase=(fb>0?0:1)^(side>0?0:1),c=(((frame||0)&3)+2*phase)&3,raised=1===c?2.8:0,sw=[-1,0,1,0][c]*.07*len,out=side*2.4,kx=hx+fb*.12*len+.5*sw,ky=hy+out,kz=.68*hipZ+raised,ax=hx+fb*.03*len+sw,ay=hy+1.25*out,az=3+raised,fz=.5+raised;strut3(e,hx,hy-side*2.2,hipZ,hx,hy+side*1.6,hipZ,2.7,jointMat);e.push(P_(DOME(1.5,.8,10),hx,hy+side*1.7,hipZ-.9,glow,{e:1}));strut3(e,hx,hy,hipZ,kx,ky,kz,1.9,legMat);strut3(e,L(hx,kx,.12),L(hy,ky,.12),L(hipZ,kz,.12),L(hx,kx,.72),L(hy,ky,.72),L(hipZ,kz,.72),2.5,armMat);strut3(e,hx-fb*1.2,hy+side*2.3,hipZ-1.2,kx-fb*.6,ky+side*1.9,kz+1.4,.5,"steel");strut3(e,hx-fb*1.2,hy+side*2.3,hipZ-1.2,L(hx-fb*1.2,kx,.45),L(hy+side*2.3,ky+side*1.9,.45),L(hipZ-1.2,kz+1.4,.45),.8,"darkmetal");strut3(e,kx,ky-side*1.9,kz,kx,ky+side*1.9,kz,1.75,jointMat);e.push(P_(DOME(.9,.6,8),kx+fb*1.1,ky+side*1.4,kz+.9,glow,{e:1}));strut3(e,kx,ky,kz,ax,ay,az,1.35,legMat);strut3(e,L(kx,ax,.08),L(ky,ay,.08),L(kz,az,.08),L(kx,ax,.66),L(ky,ay,.66),L(kz,az,.66),1.85,armMat);e.push(P_(DOME(1.6,1.4,10),ax,ay,az-.9,jointMat));strut3(e,ax,ay,az,ax,ay,fz+.6,1,"darkmetal");e.push(P_(CYL(2.2,.9,10),ax,ay,fz-.4,footMat));for(const a of[-.55,0,.55])e.push(P_(BOXM(4.4,1.3,1.2,.45),ax+Math.cos(a)*2.9,ay+Math.sin(a)*2.9,fz-.3,footMat,{r:a})),e.push(P_(CONE(.7,.1,1.4,6),ax+Math.cos(a)*5.1,ay+Math.sin(a)*5.1,fz-.3,"steel",{ty:PI2,r:a}));e.push(P_(BOXM(2.6,1.5,1.1,.4),ax-2.3,ay,fz-.3,footMat))}}function titanLegsOrganic(e,len,wid,standH,frame,legMat,jointMat){const hipZ=standH,spanX=.3*len,hipY=.5*wid-1;for(const side of[-1,1])for(const fb of[-1,1]){const hx=fb*spanX,hy=side*hipY,phase=(fb>0?0:1)^(side>0?0:1),c=(((frame||0)&3)+2*phase)&3,raised=1===c?3:0,sw=[-1,0,1,0][c]*.07*len,kx=hx+.5*sw,fx=hx+sw,kz=.55*hipZ+.4*raised,fz=2+raised;e.push(P_(DOME(3.1,1.6,10),hx,hy,hipZ+.6,legMat));e.push(P_(DOME(2.7,2.2,10),hx,hy,hipZ,legMat));strut3(e,hx,hy,hipZ,kx,hy,kz,1.6,legMat);e.push(P_(CYL(1.3,1.9,8),kx,hy,kz,jointMat,{e:1}));strut3(e,kx,hy,kz,fx,hy,fz,1.05,legMat);e.push(P_(DOME(1.8,1.3,8),fx,hy,fz,legMat));e.push(P_(CYL(.65,1.1,6),fx,hy,fz+1.2,jointMat,{e:1}));for(const a of[-.6,0,.6])e.push(P_(CONE(.7,.08,3.4,6),fx+Math.cos(a)*1.2,hy+Math.sin(a)*1.2,fz-.4,"bone",{ty:PI2-.2,r:a}));strut3(e,hx,hy+.9*Math.sign(hy),hipZ+.4,kx,hy+.9*Math.sign(hy),kz+.4,.28,jointMat);e.push(P_(CONE(.8,.1,3,6),kx,hy,kz+1.4,"bone",{ty:-.5}))}}function titanDugIn(e,rad,z){sandbagRing(e,rad,z)}function titanVanguard(t,dep){const e=[],len=34,wid=19,standH=dep?TITAN_DEP:TITAN_STAND,hullH=8.5;dep?titanDugIn(e,.62*len,1.4):titanLegsMech(e,len,wid,standH,t,"armor2","gunmetal","dark2","arcbolt","body");const bz=standH,top=bz+hullH,cz=bz+hullH/2;e.push(P_(BOXM(.6*len,.6*wid,2,.6),0,0,bz-1.6,"dark2"));e.push(P_(SLAB(hullProfile(.92*len,wid,"vanguard"),.58*hullH,2.2,"tiA1"),0,0,bz,"body"));e.push(P_(SLAB(hullProfile(.76*len,.84*wid,"vanguard"),.42*hullH+.1,1.7,"tiA2"),-.04*len,0,bz+.58*hullH,"armor2"));e.push(P_(SLAB(roundRectProfile(.5*len,.62*wid,2.6,2),3,.9,"tiA3"),-.06*len,0,top,"trim"));for(const sgn of[-1,1]){e.push(P_(BOXM(.56*len,.3,.45,.1),-.02*len,sgn*(.5*wid+.08),bz+.46*hullH,"arcbolt",{e:1})),e.push(P_(BOXM(.34*len,.3,.35,.1),-.1*len,sgn*(.42*wid+.1),top+.2,"arcbolt",{e:1}));const sy=sgn*(.5*wid+3);e.push(P_(BOXM(10.5,4.8,4.6,1.4),1.5,sy,cz,"armor3"));for(let k=0;k<3;k++)e.push(P_(BOXM(.55,4.4,1.1,.2),-1.5+2.2*k,sy,cz+2.8,"darkmetal"));e.push(P_(CYL(1.05,12.5,10),5.5,sy,cz,"gunmetal",{ty:PI2}));for(const dz of[-1.3,1.3])e.push(P_(BOXM(11,.5,.45,.12),11.5,sy,cz+dz,"arcbolt",{e:1}));e.push(P_(CYL(1.7,1.4,10),17.2,sy,cz,"darkmetal",{ty:PI2}));e.push(P_(CYL(1.2,.4,10),18.6,sy,cz,"crystal",{ty:PI2,e:1}));e.push(P_(CYL(2.1,4.4,14),-.46*len-1,sgn*.22*wid,cz+1.2,"darkmetal",{ty:-PI2}));e.push(P_(CYL(1.55,.5,12),-.46*len-5.3,sgn*.22*wid,cz+1.2,"arcbolt",{ty:-PI2,e:1}));acUnit(e,.08*len,sgn*.34*wid,top+3,4,3,0)}const hx=.5*len+2.2;e.push(P_(SLAB(hullProfile(10,9.6,"vanguard"),4.6,1.8,"tiAh"),hx,0,bz+1.6,"armor3"));e.push(P_(WEDGE(7,8,2.6,.8),hx-.6,0,bz+6.2,"armor2"));e.push(P_(DOME(2.6,1.8,12),hx-1.2,0,bz+7.4,"glass",{e:1}));e.push(P_(BOXM(.9,7.8,1.1,.2),hx+4.6,0,bz+4.3,"arcbolt",{e:1}));const chx=hx+3.4,chz=bz+1;e.push(P_(BOXM(3.6,4.6,2.2,.5),chx,0,chz,"gunmetal"));for(const dy of[-1,1])e.push(P_(CYL(.42,6.4,8),chx+1.8,dy,chz+.3,"dark2",{ty:PI2})),e.push(P_(CYL(.55,.9,8),chx+8,dy,chz+.3,"glow",{ty:PI2,e:1}));e.push(P_(CYL(.18,8,5),-.32*len,.3*wid,top+3,"dark2")),e.push(P_(DOME(.4,.35,6),-.32*len,.3*wid,top+11,"red",{e:1}));ladder(e,-.44*len,.5*wid,bz+1.5,hullH+2.5,0);seams(e,.6*len,.7*wid,bz+1,"dark");return scale2(e)}function titanTurretVanguard(dep){const a=[],baseZ=24.6-(dep?TITAN_DROP:0);a.push(P_(CYL(2.2,1.2,16),0,0,baseZ,"dark2"));a.push(P_(CYL(1.8,1.6,16),0,0,baseZ+1.2,"body"));const podZ=baseZ+2.8;a.push(P_(BOXM(4.4,4.8,1.8,.5),0,0,podZ+.9,"gunmetal"));for(const yOff of[-1.4,0,1.4])for(const xOff of[-1.6,.8])a.push(P_(CYL(.38,1.7,8),xOff,yOff,podZ+1.8,"darkmetal",{ty:1.34})),a.push(P_(CYL(.46,.5,8),xOff-1.5,yOff,podZ+2.2,"crystal",{ty:1.34,e:1}));a.push(P_(CYL(.3,1.3,6),0,0,podZ+.2,"steel"));return scale2(a)}function titanLegion(t,dep){const e=[],len=35,wid=20,standH=dep?TITAN_DEP:TITAN_STAND,hullH=9;dep?titanDugIn(e,.62*len,1.4):titanLegsMech(e,len,wid,standH,t,"armor3","rust","dark2","glow","body");const bz=standH,top=bz+hullH,cz=bz+hullH/2;e.push(P_(BOXM(.62*len,.62*wid,2,.4),0,0,bz-1.6,"dark2"));e.push(P_(BOXM(.9*len,wid,.6*hullH,.5),0,0,bz,"body"));e.push(P_(WEDGE(.2*len,.94*wid,.6*hullH,.25*hullH),.5*len,0,bz,"body"));e.push(P_(BOXM(.74*len,.88*wid,.4*hullH,.4),-.05*len,0,bz+.6*hullH,"armor2"));e.push(P_(BOXM(.52*len,.66*wid,3.4,.4),-.06*len,0,top,"trim"));for(const sgn of[-1,1]){for(let k=0;k<6;k++)e.push(P_(BOXM(.12*len,.7,2.2,.2),(-.36+.14*k)*len,sgn*(.5*wid+.35),bz+1.6,k%2?"armor3":"armor2"));e.push(P_(BOXM(.5*len,.3,.45,.1),0,sgn*(.44*wid+.2),bz+.6*hullH+.2,"glow",{e:1}));bolts(e,0,sgn*.42*wid,top-.5,.7*len,0,6,"steel");const sy=sgn*(.5*wid+3.2);e.push(P_(BOXM(10.5,5.4,5.6,.4),1,sy,cz,"armor2"));e.push(P_(BOXM(10.5,1,5.8,.2),1,sy+sgn*2.8,cz,"armor3"));e.push(P_(CYL(1.6,12,6),5,sy,cz,"darkmetal",{ty:PI2}));e.push(P_(BOXM(2.6,3.8,2.6,.3),17.5,sy,cz,"rust"));e.push(P_(BOXM(.5,3.9,.9,.1),18.2,sy,cz,"dark2"));e.push(P_(CYL(1,.6,6),19,sy,cz,"glow",{ty:PI2,e:1}));const sx=-.42*len;e.push(P_(CYL(1.7,6,10),sx,sgn*.26*wid,top+.5,"rust"));e.push(P_(CYL(2,1.1,10),sx,sgn*.26*wid,top+6.5,"darkmetal"));e.push(P_(CYL(1.1,.8,8),sx,sgn*.26*wid,top+7.4,"glow",{e:1}))}const hx=.5*len+2.6;e.push(P_(BOXM(8,10,7.4,.6),hx,0,bz+1.4,"armor2"));e.push(P_(WEDGE(3,10.2,7.4,4.6),hx+5.3,0,bz+1.4,"armor3"));e.push(P_(BOXM(1.2,7.4,1.3,.2),hx+6.4,0,bz+5.2,"glow",{e:1}));e.push(P_(BOXM(6.6,8.6,1.2,.3),hx-.4,0,bz+8.8,"dark2"));e.push(P_(BOXM(.35,2.6,2.6,.1),hx+6.9,0,bz+2.2,"red",{e:1}));const chx=hx+3.6,chz=bz+1;e.push(P_(BOXM(4,5.4,2.6,.3),chx,0,chz,"gunmetal"));for(const dy of[-1.3,1.3])e.push(P_(CYL(.55,7,8),chx+2,dy,chz+.4,"darkmetal",{ty:PI2})),e.push(P_(BOXM(1.4,1.4,1.4,.2),chx+9.2,dy,chz+.4,"rust"));ladder(e,-.45*len,.52*wid,bz+1.5,hullH+2.8,0);pipeRun(e,.1*len,.46*wid,-.2*len,.46*wid,top+.5,.7,"rust");seams(e,.6*len,.7*wid,bz+1,"dark2");return scale2(e)}// Legion Juggernaut: a titan-sized super-heavy tank. Eight-wheel tracks and
// skirts, a broad armoured turret with twin long cannons and a missile pod
// on each flank (for aircraft); no machine gun.
const JUG_Z=21,JUG_T=19;
function juggernautHull(t){const p=tankHull(64,30,t,{wheels:8,skirt:!0,th:13,trkW:12,fac:"legion"});
for(const sg of[-1,1]){for(const xo of[-22,-8,6,20])p.push(P_(BOXM(5,.6,3.4,.3),xo,sg*15.6,9,"darkmetal",{r:sg*.06}));p.push(P_(BOXM(58,1.2,1.4,.3),0,sg*15.2,12.6,"rust"))}
p.push(P_(BOXM(10,22,3,.8),-24,0,13,"darkmetal"));for(const sy of[-6,0,6])p.push(P_(BOXM(8,3.2,.6,.2),-24,sy,16,"gunmetal"));
for(const sy of[-1,1])p.push(P_(CYL(1.6,5,8),-30,sy*8,14,"rust")),p.push(P_(CYL(1.9,.8,8),-30,sy*8,19,"dark2"));
for(const sy of[-1,1])p.push(P_(CYL(1.1,.7,8),31.5,sy*10,10,"lightY",{e:1,ty:PI2}));
return p}
function juggernautTurret(){const a=[],z=JUG_Z,P2=PI2,TL=30,TW=22,tx=-2,MX=tx+.5*TL,BL=30;
a.push(P_(CYL(.42*TW,2.4,14),tx,0,z-2.2,"dark"));
a.push(P_(SLAB(turretProfile(TL,TW,"legion"),7,2.2,"jugT"),tx,0,z,"body"));
a.push(P_(SLAB(turretProfile(.7*TL,.72*TW,"legion"),1.6,.7,"jugT2"),tx-.12*TL,0,z+7,"armor2"));
a.push(P_(BOXM(5,.62*TW,6.4,1.2),MX,0,z+.5,"gunmetal"));
for(const sy of[-4.4,4.4]){const bz=z+3.8;
a.push(P_(CYL(2.6,7,12),MX+1.5,sy,bz,"darkmetal",{ty:P2,b:1}));
a.push(P_(CYL(1.75,BL,12),MX+1.5,sy,bz,"gunmetal",{ty:P2,b:1}));
for(const f of[.35,.62])a.push(P_(CYL(2.05,2.4,12),MX+1.5+BL*f,sy,bz,"armor3",{ty:P2,b:1}));
a.push(P_(CYL(2.4,4.6,12),MX+1.5+BL-3.4,sy,bz,"steel",{ty:P2,b:1}));
a.push(P_(BOXM(1.6,5.6,5,.3),MX+1.5+BL+.8,sy,bz-2.5,"gunmetal",{b:1}))}
for(const sy of[-1,1]){const px=tx-.1*TL,py=sy*(.5*TW+3.4),pz=z+1;
a.push(P_(BOXM(3.4,3,4,.5),px,sy*(.5*TW+.8),pz+1,"darkmetal"));
a.push(P_(BOXM(12,5.6,6.4,.8),px,py,pz,"body"));
a.push(P_(BOXM(12.3,5.9,.8,.25),px,py,pz+6.4,"rust"));
for(const oy of[-1.3,1.3])for(const oz of[1.7,4.3])a.push(P_(CYL(1,.8,8),px+5.8,py+oy,pz+oz,"dark2",{ty:P2})),a.push(P_(CYL(.65,.45,8),px+6.4,py+oy,pz+oz,"red",{ty:P2,e:1}))}
a.push(P_(CYL(2.2,1.8,10),tx-.2*TL,-.2*TW,z+8.7,"darkmetal"));a.push(P_(DOME(1.9,1.2,10),tx-.2*TL,-.2*TW,z+10.5,"gunmetal"));
for(const k of[-1,1])a.push(P_(BOXM(1.2,1,1,.2),tx-.2*TL+1.9,-.2*TW+k*1.2,z+9.7,"glassdark",{e:1}));
a.push(P_(CYL(.28,8,5),tx-.38*TL,.3*TW,z+8.7,"darkmetal"));
for(const k of[-1,1])for(let q=0;q<3;q++)a.push(P_(CYL(.6,1.9,6),tx+.12*TL-1.7*q,k*.44*TW,z+7,"dark2",{ty:-.9}));
return a}
function titanTurretLegion(dep){const a=[],baseZ=24.9-(dep?TITAN_DROP:0);a.push(P_(BOXM(4.2,3.7,1.3,.3),0,0,baseZ+.65,"dark2"));a.push(P_(BOXM(3.3,3.1,1.6,.3),0,0,baseZ+2.1,"body"));const podZ=baseZ+2.9;a.push(P_(BOXM(4.4,4.8,1.7,.3),0,0,podZ+.85,"armor3"));for(const yOff of[-1.4,0,1.4])for(const xOff of[-1.6,.8])a.push(P_(CYL(.4,1.8,4),xOff,yOff,podZ+1.7,"darkmetal",{ty:1.3})),a.push(P_(CYL(.46,.48,4),xOff-1.6,yOff,podZ+2.1,"rust",{ty:1.3,e:1}));bolts(a,0,0,podZ+1.7,4,0,6,"steel");return scale2(a)}function titanSyndicate(t,dep){const e=[],len=34,wid=20,standH=dep?TITAN_DEP:TITAN_STAND,br=16&(t||0)?1:0,bs=1+.06*br,bz=standH,cz=bz+.5,L=(a,b,k)=>a+(b-a)*k;
...
return scale2(e)}function titanTurretSyndicate(dep){const a=[],baseZ=24.4-(dep?TITAN_DROP:0);a.push(P_(DOME(1.9,1,12),0,0,baseZ,"carapace2"));const podZ=baseZ+1;a.push(P_(DOME(2.2,1.7,12),0,0,podZ,"carapace"));a.push(P_(DOME(.95,.8,10),0,0,podZ+1.7,"psi",{e:1}));for(const yOff of[-1.3,0,1.3])for(const xOff of[-1.6,.8])a.push(P_(CYL(.28,1.5,7),xOff,yOff,podZ+.7,"carapace2",{ty:1.3})),a.push(P_(DOME(.32,.28,7),xOff-1.5,yOff,podZ+1.2,"bile",{e:1}));return scale2(a)}const INF_FAC={militia:"legion",flak:"legion",bombard:"legion",caustic:"legion",reaper:"legion",acolyte:"syndicate",leech:"syndicate",brood:"syndicate",vector_inf:"syndicate",phantom:"syndicate",piercer:"syndicate"};function UMODEL_(e,t,extra){_infFac=extra&&extra.fac||INF_FAC[e]||"vanguard";switch(e){case"mhq":return mcvModel(t,extra&&extra.fac);case"bastion":return bastionModel(t,extra&&extra.fac);case"titan_vanguard":return titanVanguard(t,!(!extra||!extra.dep));case"titan_legion":return juggernautHull(t);case"titan_syndicate":return titanSyndicate(t,!(!extra||!extra.dep));case"hivetrans":return harvesterModel(t,"syndicate");case"drone":return droneModel(t);case"miner_vanguard":return harvesterModel(t,"vanguard");case"miner_legion":return harvesterModel(t,"legion");case"miner_syndicate":return harvesterModel(t,"syndicate");case"warden":{if(extra&&extra.dep)return deployedPillar(PILLAR_H.warden,13,"vanguard");const p=tankHull(35,13,t,{fac:"vanguard"}),s=4.42;return p.push(P_(CYL(.35,2.4,6),.36*35,-4.6,s+6.2,"darkmetal",{ty:PI2,r:-.25})),p.push(P_(CYL(.35,2.4,6),.36*35,-3.2,s+6.2,"darkmetal",{ty:PI2,r:-.15})),p.push(P_(CYL(.35,2.4,6),.36*35,-1.8,s+6.2,"darkmetal",{ty:PI2,r:-.05})),p.push(P_(CYL(.13,9.5,5),-.4*35,4.4,s+9.8,"dark2")),p}case"fangtank":return extra&&extra.dep?deployedPillar(PILLAR_H.fangtank,13,"syndicate"):tankHull(30,13,t,{fac:"syndicate",legs:6,lo:{thick:.95,reach:1.12,high:1.05},shape:"crab"});case"mauler":{if(extra&&extra.dep)return deployedPillar(PILLAR_H.mauler,14.5,"legion");const p=tankHull(38,14.5,t,{wheels:6,fac:"legion"}),s=4.42;for(const sgn of[-1,1])p.push(P_(CYL(.55,3.2,8),-.46*38,sgn*2.6,s+7.6,"rust")),p.push(P_(CYL(.42,.7,6),-.46*38,sgn*2.6,s+10.8,"glow",{e:1})),p.push(P_(BOXM(4.2,1.2,1.6,.3),.05*38,sgn*(.5*14.5+.7),s+2.4,"darkmetal"));return p}case"colossus":{const p=tankHull(43,17.5,t,{wheels:6,skirt:!0,th:9.5,trkW:8,fac:"legion"}),s=4.94;for(const sgn of[-1,1])for(const xo of[-10,5])p.push(P_(BOXM(3.4,.5,2.4,.2),xo,sgn*(.5*17.5+.4),s+5,"darkmetal",{r:sgn*.08}));return p.push(P_(CYL(.9,.6,8),.46*43,0,s+6,"lightY",{e:1,ty:PI2})),p}case"photon":{const p=tankHull(32,13,t,{fac:"vanguard",deployed:!!(extra&&extra.dep)}),s=4.42;return p.push(P_(DOME(1.3,1.1,8),.4*32,-3,s+7,"crystal",{e:1})),p.push(P_(DOME(1.3,1.1,8),.4*32,3,s+7,"crystal",{e:1})),p.push(P_(CONE(.6,.1,4,7),-.3*32,0,s+8,"crystal",{e:1})),p}case"stinger":return tankHull(26,10,t,{fac:"syndicate",legs:4,lo:{thick:.72,reach:1.4,high:1.3},shape:"wasp"});case"overmind":return tankHull(38,18,t,{fac:"syndicate",legs:8,th:8,lo:{thick:1.7,reach:.85,high:.7},shape:"brain"});case"skirmisher":{const p=tankHull(26,12,t,{wheeled:!0,th:7,fac:"vanguard"}),s=3.64;return p.push(P_(BOXM(7,8,3,.6),-8,0,s+8.4,"armor3")),p.push(P_(BOXM(1.6,1.6,.5,.15),-6,-2.4,s+10,"darkmetal")),p.push(P_(BOXM(1.6,1.6,.5,.15),-6,2.4,s+10,"darkmetal")),p.push(P_(CYL(.1,8.5,5),-9,3,s+10.2,"dark2")),p}case"hover":{const e=[],t=30,r=13;e.push(P_(SLAB(roundRectProfile(1.02*t,r+7,5.5,3),5,1.6,"hvs"),0,0,1.2,"rubber")),e.push(P_(SLAB(hullProfile(t,r+1,"syndicate"),6.2,1.8,"hvh"),0,0,6,"armor2")),e.push(P_(WEDGE(.22*t,.84*r,5.6,2),.34*t,0,12.2,"armor")),e.push(P_(SLAB(hullProfile(.58*t,.82*r,"syndicate"),4.6,1.4,"hvd"),.04*-t,0,12.2,"armor")),e.push(P_(BOXM(.2*t,.66*r,2.4,.7),.34*-t,0,12.2,"armor3"));for(const n of[-1,1])e.push(P_(CYL(3,7,12),.36*-t,n*(.5*r+2),8,"gunmetal",{ty:-PI2})),e.push(P_(CYL(2.2,1.6,10),.36*-t-6.4,n*(.5*r+2),8,"glow",{ty:-PI2,e:1}));return e.push(P_(BOXM(.3*t,1.2,2.4,.4),0,-(.5*r+.6),12,"body")),e.push(P_(BOXM(.3*t,1.2,2.4,.4),0,.5*r+.6,12,"body")),e.push(P_(CYL(1.4,1.7,8),.4*t,.26*-r,13,"lightY",{e:1})),e.push(P_(CYL(1.4,1.7,8),.4*t,.26*r,13,"lightY",{e:1})),e}case"bulwark":{const p=tankHull(40,16.5,t,{wheels:7,skirt:!0,th:9.5,trkW:8,fac:"vanguard"}),s=4.94;return p.push(P_(BOXM(1.5,.85*16.5,4,.4),.46*40,0,s+3,"darkmetal")),p.push(P_(BOXM(3,2,2,.3),0,-(.5*16.5+1.2),s+7,"steel")),p.push(P_(BOXM(3,2,2,.3),0,.5*16.5+1.2,s+7,"steel")),p}case"hound":{const p=tankHull(24,11,t,{wheeled:!0,th:6.5,fac:"legion"}),s=3.38;return p.push(P_(CYL(1,.3,8),-.36*24,0,s+7.6,"dark2",{ty:PI2})),p.push(P_(DOME(1,.45,8),-.36*24,0,s+7.6,"gunmetal",{ty:PI2})),p.push(P_(CYL(.5,1.6,6),.42*24,-3.4,s+6.4,"lightY",{e:1})),p.push(P_(CYL(.5,1.6,6),.42*24,3.4,s+6.4,"lightY",{e:1})),p}case"sledge":{const p=tankHull(36,15,t,{wheels:6,skirt:!0,th:9,fac:"legion"}),s=4.68;for(const sgn of[-1,1])p.push(P_(BOXM(1.2,3,3.4,.3),-.48*36,sgn*5,s+1.5,"darkmetal"));return p.push(P_(BOXM(5,3,2,.3),.15*36,.5*15+.6,s+2.5,"olive")),p}case"shard":return tankHull(38,17,t,{fac:"syndicate",legs:8,th:9.5,lo:{thick:1.45,reach:1,high:.95},shape:"beetle"});case"longbow":return longbowModel(t);case"rift":return riftModel(t,extra);case"spore":{const e=[],r=32,n=14,a=8.5,dep=!!(extra&&extra.dep);dep?sandbagRing(e,.6*r,1.2):legWalker(e,r,n,a,t,6);const o=dep?.5*a-5:.5*a;dep?e.push(P_(SLAB(hullProfile(.92*r,n+2,"syndicate"),6.5,1.6,"sph"),0,0,o,"dark2")):organicBody(e,r,n+2,o-1,{},t),e.push(P_(DOME(.52*n,8,10),.1*-r,0,o+6.5,"carapace")),e.push(P_(CONE(8.5,4,9,9),.14*r,0,o+6.5,"carapace2",{ty:-.55})),e.push(P_(DOME(4.4,4,8),.2*r,0,o+13.5,"psi",{e:1}));for(let t=0;t<4;t++){const n=t*PI2+.78;e.push(P_(CYL(1.1,6,6),.1*-r+5*Math.cos(n),5*Math.sin(n),o+12.5,"psi",{e:1}))}for(let t=0;t<3;t++){const x=-.15*r+.16*r*t;e.push(P_(DOME(2.6,2,8),x,.34*-n,o+3.2,"carapace",{ty:.3})),e.push(P_(DOME(2.6,2,8),x,.34*n,o+3.2,"carapace",{ty:-.3}))}return e}case"tickdrone":{const e=[];e.push(P_(DOME(6.5,4.5,10),0,0,5,"darkmetal")),e.push(P_(CYL(6.5,5,10),0,0,3,"dark2")),e.push(P_(DOME(4.2,3,9),0,0,8.6,"rust")),e.push(P_(DOME(2.4,2,8),0,0,9.5,"glass",{e:1})),e.push(P_(CONE(1.6,.3,3,6),0,0,11.6,"glow",{e:1}));for(let r=0;r<4;r++){const n=r*PI2+.78,a=7.2*Math.cos(n),o=7.2*Math.sin(n),s=2.4*(1&t?r%2:1-r%2);strut3(e,.8*a,.8*o,4.2+.4*s,1.6*a,1.6*o,5.2+.6*s,.95,"steel"),strut3(e,1.6*a,1.6*o,5.2+.6*s,2.15*a,2.15*o,2.4+s,.8,"steel"),e.push(P_(DOME(1,.9,6),1.6*a,1.6*o,4.6+.6*s,"darkmetal")),e.push(P_(CONE(1.3,.35,4,6),2.15*a,2.15*o,.2+s,"darkmetal",{r:n})),e.push(P_(DOME(.7,.6,6),1.55*a,1.55*o,2+s,"glow",{e:1}))}return e}case"ravager":{const e=[],r=34,n=15,a=8.5,dep=!!(extra&&extra.dep),loaded=!extra||void 0===extra.l||extra.l;dep?deployedOutriggers(e,r,n,a):trackUnit(e,r,n,a,t,{wheels:5});const o=.5*a;e.push(P_(SLAB(hullProfile(.92*r,n+2,"legion"),6.5,1.6,"v3h"),0,0,o,"armor2")),e.push(P_(SLAB(roundRectProfile(.3*r,n-1,2.5,2),9,2,"v3c"),.26*-r,0,o+6.5,"body")),e.push(P_(BOXM(1.6,n-4,4.4,.6),.12*-r,0,o+9.5,"glass",{e:1})),e.push(P_(BOXM(.5*r,.8*n,2.6,.8),.14*r,0,o+6.5,"armor3")),e.push(P_(CYL(1.3,.52*r,8),.1*-r,-5,o+9.4,"steel",{ty:PI2})),e.push(P_(CYL(1.3,.52*r,8),.1*-r,5,o+9.4,"steel",{ty:PI2})),loaded&&(e.push(P_(CYL(4.2,.52*r,12),.1*-r,0,o+13,"white",{ty:PI2})),e.push(P_(CONE(4.2,1.2,7,10),.42*r,0,o+13,"red",{ty:PI2})),[0,1,2,3].forEach(t=>{const n=t*PI2+.78;e.push(P_(BOXM(6,1,4.4,.4),.06*-r,4.2*Math.cos(n),o+13+4.2*Math.sin(n),"white",{r:0}))}));return e.push(P_(CYL(1.6,7,8),.42*-r,0,o+7,"rust",{ty:-PI2})),e}case"rifleman":{const p=infantry(t,{helmet:"body",pack:"armor3",visor:"glass"});return p.push(P_(BOXM(1.8,1,1.4,.2),1.6,0,8.6,"dark2")),p}case"lancer":{const p=infantry(t,{helmet:"body",pack:"armor3",weapon:"rocket"});return p.push(P_(CYL(.5,4,6),-3.2,-1,9,"darkmetal")),p.push(P_(CYL(.5,4,6),-3.2,1,9,"darkmetal")),p}case"militia":return infantry(t,{helmet:"bark",pack:"dark",gear:"dark",visor:"glow",beret:"bark2"});case"flak":{const p=infantry(t,{helmet:"armor3",pack:"steel",gear:"dark",weapon:"launcher",visor:"glow"});return p.push(P_(BOXM(1.6,1.4,2.2,.3),0,3.6,11.5,"darkmetal")),p}case"acolyte":return infantry(t,{cloth:"body",hood:"carapace2",tank:"psi",weapon:"flamer",visor:"psi"});case"vector_inf":return infantry(t,{cloth:"carapace2",hood:"body",weapon:"long",pack:"carapace",visor:"psi"});case"engineer":{const p=infantry(t,{cloth:"gold",helmet:"white",pack:"steel",weapon:"none",visor:"glass"});return p.push(P_(BOXM(.4,3.2,.4,.1),1.8,-1.4,9,"darkmetal",{r:.4})),p}case"rogue":return infantry(t,{cloth:"olive",gear:"dark",beret:"olive"});case"caustic":return infantry(t,{cloth:"green",helmet:"green",tank:"green",weapon:"flamer",visor:"glow"});case"phaser":{const p=infantry(t,{cloth:"armor",helmet:"steel",pack:"crystal",weapon:"long",visor:"crystal"});return p.push(P_(CYL(1,2.5,8),-2.8,0,12.5,"arcbolt",{e:1})),p}case"operative":{const hw=extra&&"mg"===extra.heroMode?"rifle":"long",p=infantry(t,{cloth:"armor2",beret:"red",helmet:"dark2",weapon:hw,visor:"red"});return p.push(P_(BOXM(2.6,3.4,3,.4),-2.8,-2.6,8,"dark2")),p.push(P_(CYL(.25,3,6),2.4,-2,6,"steel",{ty:PI2,r:.3})),p}case"reaper":{const hw=extra&&"mg"===extra.heroMode?"rifle":"long",p=infantry(t,{cloth:"rust",beret:"dark2",helmet:"dark2",weapon:hw,visor:"red"});return p.push(P_(BOXM(1,5.6,1,.15),1.6,-2.4,7,"darkmetal",{r:.5})),p.push(P_(BOXM(1,5.6,1,.15),1.6,2.4,7,"darkmetal",{r:-.5})),p}case"phantom":{const hw=extra&&"mg"===extra.heroMode?"rifle":"long",p=infantry(t,{cloth:"carapace2",hood:"carapace",weapon:hw,visor:"crystal"});return p.push(P_(DOME(.5,.4,6),0,0,17.6,"psi",{e:1})),p}case"marksman":{const p=infantry(t,{cloth:"armor3",helmet:"olive",pack:"olive",weapon:"launcher",visor:"glass"});return p.push(P_(BOXM(2.6,4.4,3,.4),-2.6,0,8.5,"olive")),p}case"vindicator":{const p=infantry(t,{cloth:"arcbolt",helmet:"steel",pack:"crystal",weapon:"long",visor:"arcbolt"});return p.push(P_(CYL(.7,3,7),-2.8,-1.2,9,"arcbolt",{e:1})),p.push(P_(CYL(.7,3,7),-2.8,1.2,9,"arcbolt",{e:1})),p}case"bombard":{const p=infantry(t,{cloth:"armor3",helmet:"armor3",pack:"red",weapon:"rocket",visor:"glow"});return p.push(P_(CYL(.7,4.6,7),-3,-1.3,9,"darkmetal",{r:-.2})),p.push(P_(CYL(.7,4.6,7),-3,1.3,9,"darkmetal",{r:-.2})),p}case"leech":return infantry(t,{cloth:"carapace2",hood:"carapace",tank:"crystal",weapon:"launcher",visor:"crystal"});case"brood":return infantry(t,{cloth:"carapace",hood:"body",weapon:"none",visor:"psi"}).map(e=>Object.assign({},e,{sx:1.3*(e.sx||1),sy:1.3*(e.sy||1),sz:1.2*(e.sz||1),z:1.2*e.z}));case"skyjack":{const e=infantry(0,{cloth:"body",helmet:"glass",weapon:"none"}),f=t?1:0;e.push(P_(BOXM(3.4,6.4,6.8,.8),-3.4,0,8.6,"armor3"));for(const s of[-1,1]){e.push(P_(CYL(1.6,8,10),-3.8,s*3.2,5.2,"steel"));e.push(P_(CYL(1.9,1,10),-3.8,s*3.2,5.2,"darkmetal"));e.push(P_(CONE(1.5,.2,f?5.2:3.6,8),-3.8,s*3.2,5.2,"glow",{e:1,tx:Math.PI}));e.push(P_(CONE(.8,.1,f?3:2,6),-3.8,s*3.2,4.6,"lightY",{e:1,tx:Math.PI}));e.push(P_(SLAB(filletPoly([[1.5,0],[-1.5,s*7],[-3.5,s*7],[-2.5,0]],.4,2),.4,.1,"sjW"+s),-3.8,s*3.4,13,"body"));e.push(P_(BOXM(.5,.5,.5,.1),-6.4,s*10.4,13.2,s>0?"green":"red",{e:1}))}for(const s of[-1,1])e.push(P_(CYL(.9,7,8),1.4,s*2.8,9.6,"darkmetal",{ty:PI2})),e.push(P_(CYL(1.1,.8,8),8.4,s*2.8,9.6,"gunmetal",{ty:PI2}));e.push(P_(BOXM(2,5.4,1.6,.3),2.6,0,8.8,"gunmetal"));return scaleN(e,1)}case"hornet":return hornetModel(t);case"eagle":return eagleModel(t);case"kestrel":return kestrelModel(t);case"jackal":return jackalModel(t);case"locust":return locustModel(t);case"wisp":{const e=[],ph=t?.39:0;e.push(P_(CONE(14,9.5,3.4,20),0,0,0,"dark"));e.push(P_(CONE(9.5,5,2.6,18),0,0,3.4,"body"));e.push(P_(CYL(14.4,1,24),0,0,-.4,"trim"));e.push(P_(DOME(6,4.6,14),0,0,5.6,"psi",{e:1}));e.push(P_(DOME(3.2,2.8,12),0,0,9.6,"glassdark",{e:1}));e.push(P_(CONE(7.5,11,2.6,18),0,0,-2.8,"dark"));e.push(P_(CONE(4,1.4,3.4,12),0,0,-6.2,"psi",{e:1}));for(let r=0;r<8;r++){const n=.785*r+ph;e.push(P_(CYL(1.3,1.4,8),12.3*Math.cos(n),12.3*Math.sin(n),-.6,r%2?"crystal":"psi",{e:1}))}for(let r=0;r<3;r++){const n=2.094*r+.5;e.push(P_(WEDGE(6,.6,1.4,3.6),9*Math.cos(n),9*Math.sin(n),2.6,"carapace2",{r:n+Math.PI}));e.push(P_(CONE(.8,.1,5,6),6*Math.cos(n),6*Math.sin(n),-4.4,"carapace2",{tx:Math.PI}))}return e}case"leviathan":{const e=[],prof=[[-52,.6],[-44,9.5],[-34,18],[-21,26],[-7,32],[7,35],[21,31.5],[33,24],[43,13.5],[49,0]];for(let i=0;i<prof.length-1;i++){const x0=prof[i][0],r0=prof[i][1],x1=prof[i+1][0],r1=prof[i+1][1];e.push(P_(CONE(r0,r1,x1-x0,20),x0,0,12,"body",{ty:PI2}))}for(let i=1;i<prof.length-1;i++){const x=prof[i][0],r=prof[i][1];e.push(P_(CYL(r+1.3,2.2,20),x,0,12,"dark2",{ty:PI2}))}e.push(P_(BOXM(66,5,3.4,.6),-8,0,45,"trim"));for(let i=0;i<8;i++){const x=-42+11*i;e.push(P_(BOXM(2.6,2.6,2.8,.3),x,0,47,"dark"))}e.push(P_(BOXM(14,10,7,.9),9,0,44,"dark")),e.push(P_(BOXM(12,8,2.4,.3),9,0,48.4,"glass",{e:1})),bolts(e,9,-5.2,44,6.5,0,4,"steel"),bolts(e,9,5.2,44,6.5,0,4,"steel"),e.push(P_(CYL(.9,7,6),9,0,50.6,"trim")),e.push(P_(CYL(2,1.3,8),9,0,58,"glow",{e:1}));e.push(P_(DOME(9,7.5,12),42,0,12,"dark2")),e.push(P_(CYL(.8,7,6),47,0,12,"trim",{ty:PI2})),e.push(P_(CYL(1.4,1.2,8),54.3,0,12,"glow",{e:1,ty:PI2}));for(const r of[-37,37]){e.push(P_(CYL(6,20,12),-10,r,11,"steel",{ty:PI2})),e.push(P_(CYL(6.6,3.4,12),-19.5,r,11,"darkmetal",{ty:PI2})),e.push(P_(BOXM(3,r>0?24:5.4,r>0?5.4:24,.6),-6,r,11,"darkmetal")),e.push(P_(CYL(4.6,2,10),-21.2,r,11,"glow",{e:1,ty:PI2})),e.push(P_(BOXM(12,4,4,.5),5,r,11,"trim"))}const ph=t?.39:0;e.push(P_(WEDGE(26,1.8,24,4),-43,0,19,"dark")),e.push(P_(WEDGE(26,1.8,20,4),-43,0,5,"dark",{tx:Math.PI})),e.push(P_(WEDGE(26,1.8,22,4),-43,-8,12,"dark",{tx:PI2})),e.push(P_(WEDGE(26,1.8,22,4),-43,8,12,"dark",{tx:-PI2})),e.push(P_(BOXM(3,2,20,.3),-54.6,0,23,"trim")),e.push(P_(BOXM(3,2,16,.3),-54.6,0,-15,"trim")),e.push(P_(BOXM(3,18,2,.3),-54.6,-21,11,"trim")),e.push(P_(BOXM(3,18,2,.3),-54.6,21,11,"trim")),e.push(P_(CYL(1,1,8),-54,-30,12,"red",{e:1,tx:PI2})),e.push(P_(CYL(1,1,8),-54,31,12,"green",{e:1,tx:PI2})),e.push(P_(CYL(1,1.2,8),-54,0,43,"white",{e:1})),e.push(P_(CONE(2.8,.6,5,12),-52,0,12,"darkmetal",{ty:-PI2}));for(let k=0;k<4;k++)e.push(P_(BOXM(.9,1.8,12,.2),-55.5,0,12,"steel",{tx:k*PI2+ph}));for(const r of[-37,37]){e.push(P_(CONE(3.2,.6,3.4,12),10,r,11,"trim",{ty:PI2}));for(let k=0;k<3;k++)e.push(P_(BOXM(.7,1.6,9.5,.15),12,r,11,"steel",{tx:k*2.094+ph}))}e.push(P_(SLAB(roundRectProfile(34,17,5,3),12,2.6,"kirg"),5,0,-19,"dark")),e.push(P_(BOXM(31,14,2,.6),5,0,-19.3,"trim"));for(let i=0;i<5;i++){const x=-9+7*i;e.push(P_(BOXM(4,14,2.1,.4),x,0,-13.2,"arcbolt",{e:1}))}return e.push(P_(DOME(6,5.4,10),13,0,-8,"glass",{e:1})),e}case"piercer":{const p=infantry(t,{cloth:"carapace2",hood:"carapace",weapon:"rocket",visor:"psi"});return p.push(P_(DOME(1,.8,7),-3.4,0,11.5,"bile",{e:1})),p}case"restorer":return restorerModel(t);case"salvager":return salvagerModel(t);case"cultivator":return cultivatorModel(t);}if("interceptor"===e){const p=boatHull(30,11,t,{fac:"vanguard"}),th=6.5;return p.push(P_(CYL(.15,5,6),.1*30,0,th+6,"dark2")),p.push(P_(BOXM(.7,.7,.4,.1),.1*30,0,th+6.6,"gunmetal")),p.push(P_(CYL(.3,2.2,6),.44*30,-1.6,th+2.2,"gunmetal",{ty:PI2})),p.push(P_(CYL(.3,2.2,6),.44*30,1.6,th+2.2,"gunmetal",{ty:PI2})),p}if("riverine"===e){const p=boatHull(30,11,t,{fac:"legion"}),th=6.5;for(const sgn of[-1,1])p.push(P_(BOXM(.5*30,.06*11,1.2,.2),-.05*30,sgn*.4*11,th+2.6,"darkmetal"));return p.push(P_(CYL(.5,3,7),-.15*30,0,th+2.5,"rust")),p}if("polyp"===e){const p=boatHull(30,11,t,{fac:"syndicate"}),th=6.5;for(const sgn of[-1,1])p.push(P_(DOME(.6,.5,7),.15*30,sgn*.32*11,th+2.2,"psi",{e:1}));return p}if("frigate"===e){const p=boatHull(44,15,t,{twin:!0,fac:"vanguard"}),th=6.5;return p.push(P_(DOME(1.6,1.3,10),.3*44,0,th+4.5,"glass",{e:1})),p.push(P_(BOXM(6,5,.6,.2),-.05*44,0,th+2.3,"darkmetal")),p}if("destroyer"===e){const p=boatHull(44,15,t,{twin:!0,fac:"legion"}),th=6.5;for(const sgn of[-1,1])p.push(P_(CYL(.35,4,6),0,sgn*3,th+1.7,"gunmetal",{ty:PI2}));return p.push(P_(CYL(1.2,4,8),-.1*44,0,th+3.5,"rust")),p}if("kraken"===e){const p=boatHull(44,15,t,{twin:!0,fac:"syndicate"}),th=6.5;for(const sgn of[-1,1])p.push(P_(CONE(.9,.15,7,7),-.42*44,sgn*3,th+.5,"carapace2",{ty:PI2}));return p.push(P_(BOXM(20,.4,.3,.1),-.05*44,0,th+5.4,"psi",{e:1})),p}if("barracuda"===e){const L=38,p=subHull(L,12,t,{fac:"vanguard"}),rad=.42*12;p.push(P_(DOME(.62*rad,.5*rad,12),.52*L,0,rad,"glass",{e:1,ty:PI2}));for(let k=0;k<4;k++)p.push(P_(BOXM(1.6,1.6,.4,.2),-.22*L+k*2.4,0,rad+.78*rad+.6,"darkmetal"));p.push(P_(BOXM(.3*L,.3,.4,.1),-.05*L,-.46*rad,1.7*rad,"arcbolt",{e:1}));p.push(P_(BOXM(.3*L,.3,.4,.1),-.05*L,.46*rad,1.7*rad,"arcbolt",{e:1}));return p}if("nautilus"===e){const L=38,p=subHull(L,12,t,{fac:"legion"}),rad=.42*12;for(const sgn of[-1,1])p.push(P_(CYL(.34*rad,.34*L,10),.2*L,sgn*(rad+.2),.7*rad,"gunmetal",{ty:-PI2})),p.push(P_(CONE(.34*rad,.1*rad,.06*L,10),.2*L,sgn*(rad+.2),.7*rad,"rust",{ty:PI2}));p.push(P_(CYL(.5,1.8*rad,7),.1*L,0,1.8*rad,"rust"));seams(p,.3*L,.9*rad,1.78*rad+.65,"rust");return p}if("lamprey"===e){const L=38,p=subHull(L,12,t,{fac:"syndicate"}),rad=.42*12,wv=((t||0)&1)?1:-1;for(const sgn of[-1,0,1]){const y0=sgn*.35*rad,z0=rad,x1=-.62*L,y1=sgn*.9*rad+wv*.6,x2=-.8*L,y2=sgn*1.4*rad-wv*.8;strut3(p,-.5*L,y0,z0,x1,y1,z0+.1,.5,"carapace2");strut3(p,x1,y1,z0+.1,x2,y2,z0,.32,"carapace");p.push(P_(DOME(.5,.4,7),x2,y2,z0-.2,"psi",{e:1}))}for(const sgn of[-1,1])p.push(P_(DOME(.7,.6,8),.5*L,sgn*.4*rad,1.3*rad,"psi",{e:1}));return p}if("chinook"===e)return chinookModel();if("skyhauler"===e)return skyhaulerModel();if("gullet"===e)return gulletModel(t);if("lst"===e){const p=boatHull(46,17,t,{fac:"vanguard"}),th=6.5;p.push(P_(WEDGE(.16*46,.7*17,3,.4),.34*46,0,th,"steel"));for(const xo of[-8,0,8])p.push(P_(BOXM(4,4,3,.3),xo,-4,th+2.6,"rust"));return p}if("barge"===e){const p=boatHull(46,17,t,{fac:"legion"}),th=6.5;p.push(P_(CYL(.4,6,6),-.4*46,0,th+3,"darkmetal")),p.push(P_(BOXM(8,.8,.5,.15),-.4*46+3,0,th+9,"darkmetal"));for(const xo of[-6,2])p.push(P_(BOXM(4,4,3,.3),xo,4,th+2.6,"olive"));return p}if("maw"===e){const p=boatHull(46,17,t,{fac:"syndicate"}),th=6.5;p.push(P_(CONE(2.2,.3,4,9),.42*46,0,th+1,"dark",{ty:PI2})),p.push(P_(DOME(2,1.6,9),.44*46,0,th+1,"psi",{e:1}));for(const sgn of[-1,1])p.push(P_(CONE(.7,.1,5,6),-.4*46,sgn*2.5,th,"carapace2",{ty:PI2}));return p}return tankHull(30,13,t)}function UTURRET_(e,t,extra?){switch(e){case"miner_vanguard":return tankTurret(9,5.5,6.5,{pods:!0,fac:"vanguard"});case"warden":return tankTurret(11.5,9.2,20,{fac:"vanguard"});case"fangtank":return tankTurret(9.4,7.6,13,{fang:!0,fac:"syndicate"});case"mauler":return tankTurret(12.5,10.2,21,{fac:"legion"});case"colossus":return tankTurret(14.5,12.2,23,{twin:!0,fac:"legion"});case"skirmisher":{const gk=extra&&extra.gunKey,gd=gk&&UNITS[gk];if(gd&&"engineer"===gd.role){const p=tankTurret(10,7,0,{fac:"vanguard"}).filter(q=>(q.z||0)<12.5),top=12;return p.push(P_(BOXM(10,8,3.2,.6),-1,0,top,"body"),P_(BOXM(10.4,8.4,.5,.2),-1,0,top+3.2,"trim"),P_(CYL(1.6,1.6,10),2,0,top+3.6,"darkmetal"),P_(CYL(.9,5,8),2,0,top+5,"gold"),P_(BOXM(12,1.1,1.1,.2),6.8,0,top+8.6,"gold",{ty:-.32}),P_(CYL(.2,4,6),12.4,0,top+6.4,"darkmetal"),P_(BOXM(1.4,1.4,.9,.2),12.4,0,top+5.6,"steel"),P_(BOXM(3,2.2,1.6,.3),-4,2.4,top+3.7,"red"),P_(DOME(.8,.9,8),-4,-2.6,top+3.7,"glow",{e:1,a:{spin:2}}),P_(CYL(.25,3,6),-5.4,-2.6,top+3.7,"darkmetal")),p}const proj=gd&&gd.proj,variant=!gd?"pods":"flame"===proj?"flame":"flak"===proj||gd.aa?"twin":"beam"===proj?"beam":"bullet"===proj?"gat":"pods";return tankTurret(10,7,9,{fac:"vanguard",pods:"pods"===variant,gat:"gat"===variant,twin:"twin"===variant,beam:"beam"===variant,flame:"flame"===variant});}case"photon":return tankTurret(11.5,9.2,16,{beam:!0,fac:"vanguard"});case"stinger":return tankTurret(8.6,5.8,10,{gat:!0,f:t,fac:"syndicate"});case"overmind":return tankTurret(13.2,10.2,0,{dome:!0,fac:"syndicate"});case"hover":return tankTurret(9.5,6.6,8,{pods:!0,fac:"vanguard"});case"bulwark":return tankTurret(14,11.5,22,{twin:!0,fac:"vanguard"});case"hound":return tankTurret(8.5,6,11,{gat:!0,f:t,fac:"legion"});case"sledge":return tankTurret(13,10.5,26,{fac:"legion"});case"shard":return tankTurret(14.4,9.2,16,{crystal:!0,fac:"syndicate"});case"longbow":return longbowTurret();case"rift":return riftTurret(9.6);case"titan_vanguard":return titanTurretVanguard(!(!extra||!extra.dep));case"titan_legion":return juggernautTurret();case"titan_syndicate":return titanTurretSyndicate(!(!extra||!extra.dep));default:return null}}function muzzleDist(e){return"titan_legion"===e.key?46:"inf"===e.d.kind?e.d.radius+8:"ravager"===e.key||"leviathan"===e.key||"hornet"===e.key||"eagle"===e.key?e.d.radius+4:hasTurret(e.key)?e.d.radius+24:e.d.radius+10}const AFRAMES=e=>UNITS[e]&&"inf"===UNITS[e].kind?4:2,_hasT=new Map,_hasBT=new Map;function hasTurret(e){let t=_hasT.get(e);return void 0===t&&(t=!!UTURRET_(e,0),_hasT.set(e,t)),t}function hasBTurret(e,t){const r=e+"|"+t;let n=_hasBT.get(r);return void 0===n&&(n=!!BTURRET_(e,t,0),_hasBT.set(r,n)),n}function treeModel(e){const t=[];e=e|0;const bark=e%2?"bark2":"bark",R=(k)=>{const x=Math.sin(12.9898*k+78.233*e)*43758.5453;return x-Math.floor(x)},trunk=(h,r0,r1,lx,ly?)=>{taper3(t,0,0,-.6,lx||0,ly||0,h,r0,r1,bark);for(let k=0;k<4;k++){const a=k*1.571+R(k)*.8;taper3(t,Math.cos(a)*r0*.3,Math.sin(a)*r0*.3,1.8,Math.cos(a)*r0*2.3,Math.sin(a)*r0*2.3,-.4,r0*.5,.18,bark)}},blobs=(n,cx,cy,cz,sp,rz,cols,sz)=>{for(let k=0;k<n;k++){const a=k*2.399+R(k+3)*.6,rr=sp*(.35+.65*R(k+7)),z=cz+(R(k+11)-.35)*rz,s=sz*(.7+.5*R(k+13));ell(t,cx+Math.cos(a)*rr,cy+Math.sin(a)*rr,z,s,s*(.85+.3*R(k+17)),s*.8,cols[k%cols.length])}};
if(0===e||7===e){trunk(15,3.3,2.1,.8);const tops=[];for(let k=0;k<4;k++){const a=k*1.571+.5+R(k)*.4,ex=Math.cos(a)*8.5,ey=Math.sin(a)*8.5,ez=19+2.5*R(k+1);taper3(t,.6,0,10+k,ex,ey,ez,1.5,.55,bark);tops.push([ex,ey,ez])}const cols=7===e?["autumn","rust","autumn","gold","autumn"]:["green","green2","green","green3","green2"];for(const[x,y,z]of tops)blobs(3,x,y,z+2,3.5,3,cols,5.2);blobs(5,0,0,26,5,4,cols,6.4);blobs(3,0,0,31,2.6,2,7===e?["gold","autumn"]:["green3","green"],4.4)}
else if(1===e%10||5===e%10){const big=1===e%10,H=big?34:22,snowy=e>=10;trunk(H*.9,big?2.2:1.6,.7,0);const n=big?6:4;for(let k=0;k<n;k++){const f=k/(n-1),r=(big?12.5:9)*(1-.72*f)+1,z=(big?7:5)+f*(H-8),ox=(R(k)-.5)*1.2,oy=(R(k+5)-.5)*1.2;t.push(P_(CONE(r*1.04,r*.8,1.6,11),ox,oy,z-1.2,"green2"));t.push(P_(CONE(r,r*.22,big?8:6.5,11),ox,oy,z,"pine"));snowy&&t.push(P_(CONE(r*.86,r*.2,big?5.6:4.6,11),ox,oy,z+1.2,"white"))}t.push(P_(CONE(big?2.6:2,.2,big?6:4.5,8),0,0,H-1,snowy?"white":"pine"))}
else if(2===e){trunk(8,3,2.2,1.6,.4);taper3(t,1.2,.2,7,-3.5,2.5,13,1.4,.8,bark);taper3(t,1.4,.3,7,6,-2,12.5,1.3,.7,bark);for(let k=0;k<7;k++){const a=k*.9+R(k)*.5,rr=4+5*R(k+2);ell(t,Math.cos(a)*rr+1,Math.sin(a)*rr,14+2.2*R(k+4),7-2*R(k+6),6-1.5*R(k+8),3.2,k%2?"olive":"green2")}}
else if(3===e){taper3(t,0,0,-.6,1.4,.6,9,2.8,2.1,bark);taper3(t,1.4,.6,9,.4,-.4,17,2.1,1.2,bark);taper3(t,.4,-.4,17,1.6,.4,23,1.2,.5,bark);for(let k=0;k<4;k++){const a=k*1.571+.3,z=10+3.2*k,bx=Math.cos(a)*(6.5-k),by=Math.sin(a)*(6.5-k),bz=z+5-k*.5;taper3(t,.8,0,z,bx,by,bz,1,.35,bark);taper3(t,bx,by,bz,bx*1.35+Math.cos(a+1)*2,by*1.35+Math.sin(a+1)*2,bz+3.2,.35,.1,bark);taper3(t,bx,by,bz,bx*1.2,by*1.2,bz-1.8,.3,.1,bark)}for(let k=0;k<3;k++){const a=k*2.1;taper3(t,Math.cos(a),Math.sin(a),1.5,Math.cos(a)*6,Math.sin(a)*6,-.4,1.3,.2,bark)}}
else if(4===e){taper3(t,0,0,-.5,.6,.3,24,1.35,.6,"white");for(let k=0;k<7;k++)t.push(P_(BOXM(.9,.5,.35,.1),.6*k/7+Math.cos(k*2.3)*1.1,.3*k/7+Math.sin(k*2.3)*1.1,2+3.1*k,"dark2",{r:k*2.3}));for(let k=0;k<3;k++){const a=k*2.1+.4;taper3(t,.3,.1,12+3*k,Math.cos(a)*5,Math.sin(a)*5,19+3*k,.5,.2,"white")}blobs(10,0,0,21,4.8,8,["green3","green","green3"],3.4)}
else{trunk(6,2.1,1.5,.5);blobs(6,.4,0,9,3.8,2.6,["green2","green","green2"],4.4);blobs(2,.4,0,12.4,1.6,1,["green3"],3)}
return t}function rockModel(e){const t=[],r=3+e%3;for(let n=0;n<r;n++){const r=2.1*n+e,a=n?4.6+2.8*n%4:0;t.push(P_(DOME(6.8-1.1*n,5.6-1*n,7+n%2),Math.cos(r)*a,Math.sin(r)*a,.3*(n%2),n%2?"rock2":"rock3",{r:r,tx:.12*(n%2?1:-1),ty:.08*n}))}for(let n=0;n<2;n++){const r=3.4*n+1.7*e;t.push(P_(DOME(1.6,1.2,6),4.4*Math.cos(r),4.4*Math.sin(r),0,n%2?"rock3":"rock2",{r:r}))}t.push(P_(DOME(4.2,1.4,8),-.8,.6,4.4,"green2",{sz:.8})),t.push(P_(DOME(2.2,.9,7),2.2,-1.4,3.6,"green3"));for(let k=0;k<3;k++){const a=2.1*k+e;t.push(P_(CONE(.3,.02,2.6,4),Math.cos(a)*7.4,Math.sin(a)*7.4,0,"green3",{ty:.3,r:a}))}return t}function scrubModel(e){const t=[],cols=e%2?["olive","green2","green"]:["green","green2","green3"],n=5+e%3;for(let k=0;k<n;k++){const a=k*2.399+e,rr=k?2.2+1.4*(k%3)/2:0,s=k?2.6-.25*(k%3):3.2;ell(t,Math.cos(a)*rr,Math.sin(a)*rr,-.2,s,s*.9,s*.8,cols[k%3])}for(let k=0;k<6;k++){const a=k*1.05+.3;t.push(P_(CONE(.3,.02,3+(k%3),5),Math.cos(a)*3.8,Math.sin(a)*3.8,0,"green3",{ty:.3,r:a}))}if(e%2===0)for(let k=0;k<4;k++){const a=k*1.57+.7;t.push(P_(DOME(.5,.45,6),Math.cos(a)*2.2,Math.sin(a)*2.2,2.3+.3*(k%2),["white","gold","red","white"][(k+e)%4]))}return t}
function stoneModel(e){const t=[];for(let r=0;r<2+e%3;r++){const n=2.1*r+e;t.push(P_(DOME(2.2,1.5,6),2.6*Math.cos(n),2.6*Math.sin(n),0,r%2?"rock2":"sand",{r:n}))}return t}function fallModel(e){const t=[],h=26+10*(e%3);for(let r=0;r<3;r++)t.push(P_(CYL(2-.22*r,h-3*r,9),.9*(r-1),0,0,"glass",{r:.08*r,e:1}));return t.push(P_(DOME(6.8,2.6,12),0,0,0,"white")),t.push(P_(DOME(3.8,3,9),0,0,2.2,"arcbolt",{e:1})),t}function lampModel(){const e=[];return e.push(P_(CYL(1.4,1.6,10),0,0,0,"concrete2")),e.push(P_(CYL(.75,17,8),0,0,1.6,"darkmetal")),e.push(P_(BOXM(4.2,1,.9,.25),1.6,0,18.2,"darkmetal")),e.push(P_(BOXM(2.6,1.8,1,.3),3.4,0,17.4,"lightY",{e:1})),e}function churchModel(){const e=[],NW=36,ND=56,WH=18,TW=14,TH=58,SPH=20,l=3.6;
...
// Bonus crate: banded supply case with a glowing emblem on the lid.
function crateModel(){const e=[];e.push(P_(BOXM(14,14,10,.8),0,0,0,"olive")),e.push(P_(BOXM(14.6,2.2,10.4,.3),0,-4.2,0,"darkmetal")),e.push(P_(BOXM(14.6,2.2,10.4,.3),0,4.2,0,"darkmetal")),e.push(P_(BOXM(2.2,14.6,.6,.2),0,0,10,"darkmetal")),e.push(P_(CYL(3,.5,12),0,0,10.3,"gold",{e:1})),e.push(P_(BOXM(4,1,.4,.1),0,0,10.8,"glow",{e:1}));return e}
...
const yIn=side?-sg*16:-20,yOut=side?sg*19:20,W=Math.abs(yOut-yIn),yc=(yIn+yOut)/2;
e.push(P_(BOXM(L,W,1.7,.1),0,yc,0,ASPH));
e.push(P_(BOXM(L,W-4,2.6,.2),0,yc,-2.6,"concrete2"));
for(const gy of side?[yc-sg*6,yc+sg*7]:[-11,11])e.push(P_(BOXM(L,2.2,3.4,.2),0,gy,-5.6,"gunmetal"));
const outs=side?[sg]:[-1,1];
for(const o of outs){const yo=side?yOut:o*20;
 e.push(P_(BOXM(L,3,1.1,.2),0,yo-o*1.5,1.7,"concrete2"));
 for(const px of[-12,-4,4,12])e.push(P_(BOXM(.7,.7,3.2),px,yo-o*.9,2.8,"darkmetal"));
...
return t}
function organicCrane(e,cx,cy,z,h,arm){const mast=Math.max(h,20),jib=Math.max(arm,22),sw={orbit:.35,amp:.35,pvx:cx,pvy:cy};e.push(P_(DOME(5.4,3.4,12),cx,cy,z,"flesh"));for(let k=0;k<5;k++){const a=k*1.2566;taper3(e,cx+Math.cos(a)*3,cy+Math.sin(a)*3,z+1.6,cx+Math.cos(a)*7.5,cy+Math.sin(a)*7.5,z,1.3,.35,"flesh")}
const P=[[cx,cy,z+2],[cx+.6,cy,z+.45*mast],[cx+1.6,cy,z+.85*mast],[cx+.2*jib,cy,z+mast+4],[cx+.5*jib,cy,z+mast+3],[cx+.72*jib,cy,z+mast-.5]];for(let k=0;k<P.length-1;k++){const A=k>=2?{a:sw}:{};taper3(e,P[k][0],P[k][1],P[k][2],P[k+1][0],P[k+1][1],P[k+1][2],2.6-.35*k,2.3-.35*k,k%2?"carapace2":"carapace",A);e.push(P_(DOME(2.6-.35*k,2-.3*k,9),P[k+1][0],P[k+1][1],P[k+1][2]-.8,"flesh",A));k<4&&e.push(P_(CONE(.6,.05,2.4,6),P[k+1][0],P[k+1][1]+1.8,P[k+1][2],"bone",Object.assign({ty:.6},A)))}
taper3(e,cx+.1,cy+1.9,z+3,P[3][0],P[3][1]+1.4,P[3][2]-.6,.3,.25,"psi",{e:1});const hx=P[5][0],hz=P[5][2];taper3(e,hx,cy,hz,hx,cy,hz-.35*mast,.5,.35,"carapace2",{a:sw});for(const a of[-.6,.6])e.push(P_(CONE(.6,.05,3.6,6),hx+a*1.2,cy,hz-.35*mast,"bone",{tx:Math.PI+a*.5,a:sw}));e.push(P_(DOME(1.5,1.6,10),hx,cy,hz-.35*mast-2.8,"psi",{e:1,a:sw}))}
function crane(e,cx,cy,z,h,arm,mat){if("syndicate"===_bFac)return organicCrane(e,cx,cy,z,h,arm);
mat=mat||"gunmetal";
...
case"repair":{const cr={patrol:.12,pw:28,ph:0};if(Y){const cp={patrol:.12,pw:28,ph:6};n.push(P_(DOME(2.2,1.8,9),14,0,29.2,"flesh",{a:cr}));n.push(P_(CONE(.7,.45,8,7),14,0,21.4,"carapace2",{a:cp}));for(const a of[-.6,.6])n.push(P_(CONE(.5,.05,3,6),14+a,0,21.4,"bone",{tx:Math.PI+a*.6,a:cp}));n.push(P_(DOME(1.1,1.2,8),14,0,19.2,"psi",{e:1,a:cp}))}else{n.push(P_(BOXM(4,40,1.6,.2),14,0,31.5,"gunmetal",{a:cr}));n.push(P_(CYL(.18,9,5),14,0,22.5,"dark2",{a:{patrol:.12,pw:28,ph:7}}));n.push(P_(BOXM(2.2,2.2,1.4,.2),14,0,21.2,"lightY",{a:{patrol:.12,pw:28,ph:7}}))}for(const sy of[-19.2,19.2])spinBeacon(n,-21.8,sy,25.6,Y?"psi":"lightY");break}
...
function oreRigModel(fac,l){const n=[],r=96;if("vanguard"===fac){n.push(P_(CYL(.34*r,2.2,24),0,0,l,"armor2")),n.push(P_(CYL(.3*r,.6,24),0,0,l+2.2,"armor3"));for(let k=0;k<3;k++){const a=k*2.094+.3,tx=Math.cos(a)*.34*r,ty=Math.sin(a)*.34*r;n.push(P_(CYL(4.6,11,14),tx,ty,l,"armor")),n.push(P_(DOME(4.6,2.4,14),tx,ty,l+11,"armor2")),n.push(P_(CYL(4.7,.7,14),tx,ty,l+7.5,"arcbolt",{e:1})),strut3(n,tx*.55,ty*.55,l+4.5,tx*.9,ty*.9,l+4.5,1,"steel")}n.push(P_(CYL(.17*r,6,20),0,0,l+2.2,"armor")),n.push(P_(DOME(.17*r,.13*r,20),0,0,l+8.2,"armor2")),n.push(P_(CYL(.175*r,1.4,20),0,0,l+6,"glass",{e:1}));const sp={spin:1.8};n.push(P_(CYL(1.2,10,10),0,0,l+14,"steel")),n.push(P_(CYL(2,1,10),0,0,l+24,"darkmetal",{a:sp}));for(let k=0;k<3;k++)n.push(P_(BOXM(9,1.4,.3,.1),4.5*Math.cos(k*2.094),4.5*Math.sin(k*2.094),l+24.6,"white",{a:sp,r:k*2.094}));for(const sx of[-1,1])n.push(P_(CYL(.9,7,8),sx*6,-6,l+8,"steel")),n.push(P_(CYL(.5,5,8),sx*6,-6,l+15,"arcbolt",{e:1,a:{bob:2.4+sx*.3,ba:1.4}}));n.push(P_(CYL(.2,9,5),-8,8,l+8,"dark2")),n.push(P_(DOME(.4,.35,6),-8,8,l+17,"red",{e:1}))}
else if("legion"===fac){n.push(P_(CYL(.36*r,1.6,8),0,0,l,"concrete2"));for(let k=0;k<4;k++){const a=k*1.571+.785,x=Math.cos(a)*9,y=Math.sin(a)*9;strut3(n,x,y,l+1.6,Math.cos(a)*2,Math.sin(a)*2,l+38,.9,"rust")}for(const z of[12,22,31])n.push(P_(BOXM(12-.2*z,12-.2*z,.8,.2),0,0,l+z,"rust"));n.push(P_(BOXM(6,6,2.4,.4),0,0,l+38,"darkmetal")),n.push(P_(CYL(.7,.7,8),0,0,l+40.4,"red",{e:1,a:{spin:3}}));const px=.26*r,pb={bob:1.7,ba:3};n.push(P_(BOXM(3,3,12,.4),px,0,l+1.6,"darkmetal")),n.push(P_(BOXM(18,1.8,1.8,.3),px,0,l+13.6,"rust",{a:pb})),n.push(P_(BOXM(3.4,3,5,.4),px+9,0,l+10.4,"darkmetal",{a:pb})),n.push(P_(CYL(.3,9,6),px+10,0,l+2,"steel",{a:{bob:1.7,ba:2.2}})),n.push(P_(BOXM(4,4,4,.4),px-9,0,l+10.8,"concrete",{a:pb})),n.push(P_(BOXM(6,5,5,.6),px-6,0,l+1.6,"armor3"));stack(n,-.28*r,.22*r,l+1.6,2.4,22,"rust");for(const ty of[-1,1])n.push(P_(CYL(4,8,12),-.2*r,ty*.28*r,l+1.6,"rust")),n.push(P_(DOME(4,1.8,12),-.2*r,ty*.28*r,l+9.6,"darkmetal"));n.push(P_(BOXM(.4,3,3,.1),-.2*r-4.1,.28*r,l+4.5,"red",{e:1}))}
else{ell(n,0,0,l-.4,.3*r,.28*r,8,"flesh"),ell(n,0,0,l,.27*r,.25*r,20,"carapace2");for(let k=0;k<3;k++)ell(n,(.12-.12*k)*r,0,l+2,.1*r,.26*r-2*k,24-3*k,k%2?"carapace":"body");const mx=.06*r;taper3(n,mx,0,l+20,mx+3,0,l+36,5,3,"flesh");for(let k=0;k<3;k++)n.push(P_(CYL(5.2-.6*k,1.2,12),mx+1+k,0,l+24+4*k,"carapace",{ty:.18}));ell(n,mx+3,0,l+36,4.6,4.6,3.4,"carapace2");n.push(P_(DOME(2.6,2.4,10),mx+3,0,l+37.6,"psi",{e:1,a:{bob:1.3,ba:.9}}));for(let k=0;k<6;k++){const a=k*1.047,x=Math.cos(a)*.31*r,y=Math.sin(a)*.29*r;taper3(n,x*.8,y*.8,l+6,x*1.25,y*1.25,l-.6,1.8,.4,"flesh");n.push(P_(DOME(2.2,2,9),x,y,l+4,"bile",{e:1,a:{bob:1.1+.2*k,ba:.6}}))}for(let k=0;k<3;k++){const a=k*2.094+.4,x=Math.cos(a)*.14*r,y=Math.sin(a)*.14*r,A={orbit:.7+.2*k,amp:.3,pvx:x,pvy:y};taper3(n,x,y,l+13,x+Math.cos(a)*5,y+Math.sin(a)*5,l+22,1.1,.5,"carapace",{a:A});n.push(P_(DOME(.8,.7,7),x+Math.cos(a)*5,y+Math.sin(a)*5,l+22,"psi",{e:1,a:A}))}}
return n}
function winRow(n,x0,x1,y,z,h,face,mat?){const cnt=Math.max(1,Math.round(Math.abs(x1-x0)/7));for(let k=0;k<cnt;k++){const x=x0+(x1-x0)*(k+.5)/cnt;if("x"===face)n.push(P_(BOXM(.5,3,h,.1),y,x,z,mat||"glassdark")),n.push(P_(BOXM(.8,3.8,.5,.1),y,x,z-.5,"concrete"));else n.push(P_(BOXM(3,.5,h,.1),x,y,z,mat||"glassdark")),n.push(P_(BOXM(3.8,.8,.5,.1),x,y,z-.5,"concrete"))}}
function gableRoof(n,cx,cy,z,w,d,h,mat,alongX){if(alongX){n.push(P_(WEDGE(w,d/2,.2,h),cx,cy-d/4,z,mat,{r:Math.PI/2})),n.push(P_(WEDGE(w,d/2,.2,h),cx,cy+d/4,z,mat,{r:-Math.PI/2}))}else{n.push(P_(WEDGE(w/2,d,.2,h),cx-w/4,cy,z,mat)),n.push(P_(WEDGE(w/2,d,.2,h),cx+w/4,cy,z,mat,{r:Math.PI}))}}
function workshopModel(r,l){const n=[],w=.8*r,d=.56*r,h=16;n.push(P_(BOXM(w,d,h,.6),0,-.06*r,l,"neutral"));for(let k=0;k<4;k++){const x=-w/2+w*(k+.5)/4;n.push(P_(WEDGE(w/4,d,.5,7),x,-.06*r,l+h,"roof",{r:0})),n.push(P_(BOXM(.6,d-1,5.4,.1),x+w/8-.4,-.06*r,l+h+.4,"glassdark"))}n.push(P_(BOXM(14,.8,11,.3),-.12*r,d/2-.06*r,l,"darkmetal"));for(let k=0;k<8;k++)n.push(P_(BOXM(13.4,.9,.35,.1),-.12*r,d/2-.06*r+.1,l+1.2+1.3*k,"steel"));n.push(P_(BOXM(15,1.2,1.4,.2),-.12*r,d/2-.06*r,l+11,"red"));winRow(n,.1*r,.36*r,d/2-.06*r+.3,l+6,4,"y");n.push(P_(BOXM(.24*r,.3*r,9,.4),.36*r,.24*r,l,"concrete2")),n.push(P_(WEDGE(.24*r,.3*r,9.4,7),.36*r,.24*r,l+9,"rust"));n.push(P_(CYL(1.6,26,10),-.32*r,-.24*r,l,"rust")),n.push(P_(CYL(1.9,1.2,10),-.32*r,-.24*r,l+26,"darkmetal"));for(let k=0;k<3;k++)n.push(P_(CYL(1.6,3,10),.18*r+3.4*k,.38*r,l,k%2?"rust":"steel"));crate(n,-.36*r,.34*r,l,5,"wood",.2),crate(n,-.3*r,.36*r,l,4,"wood",.6),n.push(P_(BOXM(8,3.6,.6,.2),.04*r,.4*r,l+3.6,"wood")),n.push(P_(BOXM(.6,3,3.6,.1),.04*r-3.6,.4*r,l,"wood")),n.push(P_(BOXM(.6,3,3.6,.1),.04*r+3.6,.4*r,l,"wood"));for(const[x,y]of[[-.4*r,.1*r],[-.4*r,.18*r]])n.push(P_(CYL(2.4,1.2,12),x,y,l+.2,"rubber",{tx:Math.PI/2}));n.push(P_(BOXM(.8*r,.8,.8,.2),0,-.06*r,l+h-.4,"dark2"));return n}
function terraceModel(r,l){const n=[],cols=["#8e5a44","#a9927a","#6f7f8c"],w=.28*r,d=.5*r,h=22;for(let k=0;k<3;k++){const x=(-.3+.3*k)*r;n.push(P_(BOXM(w,d,h,.4),x,-.06*r,l,cols[k])),gableRoof(n,x,-.06*r,l+h,w+2,d+2,9,"roof",!0),n.push(P_(BOXM(2.4,2.4,6,.3),x+w*.3,-.2*r,l+h+3,"#7a4a3a")),n.push(P_(CYL(.15,6,5),x-w*.3,-.1*r,l+h+6,"dark2"));const fy=d/2-.06*r;for(const zz of[4.5,13])for(const xx of[-.25,.25])n.push(P_(BOXM(3.6,.5,4.4,.1),x+xx*w,fy+.2,l+zz,"glassdark")),n.push(P_(BOXM(4.4,.9,.5,.1),x+xx*w,fy+.4,l+zz-.4,"white"));n.push(P_(BOXM(3.4,.6,7,.2),x,fy+.3,l,"#3b2a20")),n.push(P_(BOXM(4.6,2,.8,.2),x,fy+1.3,l,"concrete")),n.push(P_(BOXM(w-2,5,.4,.1),x,fy+4.6,l,"green2")),n.push(P_(BOXM(w-2,.8,2.4,.4),x,fy+7.2,l,"green2")),n.push(P_(BOXM(1.2,1.2,1.2,.2),x-w*.3,fy+5,l+.4,"red"))}n.push(P_(BOXM(.9*r,.6,1,.2),0,-.06*r+d/2,l+h-1,"concrete2"));return n}
function bridgeHutModel(r,l){const n=[];n.push(P_(BOXM(20,16,11,.6),-2,0,l,"concrete2")),gableRoof(n,-2,0,l+11,22,18,6,"roof",!1),winRow(n,-8,4,8.2,l+5,3.4,"y"),n.push(P_(BOXM(3,.6,7,.2),6,8.1,l,"wood"));n.push(P_(BOXM(6,6,8,.3),7,-5,l+11,"concrete2")),n.push(P_(BOXM(6.6,6.6,.6,.2),7,-5,l+19,"roof"));for(const[x,y]of[[4.2,-7.8],[9.8,-7.8],[4.2,-2.2],[9.8,-2.2]])n.push(P_(CYL(.3,4,6),x,y,l+19.6,"darkmetal"));n.push(P_(BOXM(7,7,.8,.2),7,-5,l+23.6,"roof")),n.push(P_(CYL(.7,1,8),7,-5,l+24.4,"lightY",{e:1,a:{spin:3}}));n.push(P_(BOXM(2,2,5,.3),14,6,l,"darkmetal")),n.push(P_(BOXM(20,1,1,.2),24,6,l+4.4,"white",{a:{bob:.3,ba:.4}}));for(let k=0;k<4;k++)n.push(P_(BOXM(2.4,1.1,1.1,.2),17+5*k,6,l+4.35,"red",{a:{bob:.3,ba:.4}}));sandbagRing(n,14,l);n.push(P_(CYL(.3,10,6),-12,-8,l,"darkmetal")),n.push(P_(BOXM(4,.2,2.6,.05),-9.8,-8,l+7,"red",{a:{orbit:2.4,amp:.3,pvx:-12,pvy:-8}}));return n}
function oilDerrickModel(r,l){const n=[];ell(n,4,6,l-.2,9,7,.3,"black");
// Pumpjack: the walking beam and horsehead rock about the Samson-post bearing,
// the crank and counterweights turn continuously, the polish rod strokes in
// step with the horsehead. Anim "rock" rotates parts about a shared pivot.
const W=1.1,px=-4,PZ=l+15,Y="#c9a227",rk={rock:W,ra:.26,rpx:px,rpz:PZ},cz=l+6.2,cx=px-11,cr={rock:W,rph:-1.5708,rpx:cx,rpz:cz};
n.push(P_(BOXM(34,8,1.2,.3),px,0,l,"concrete2"));
for(const sy of[-1,1]){strut3(n,px-3.5,sy*3,l+1.2,px,sy*.9,PZ-.4,.55,"darkmetal");strut3(n,px+3.5,sy*3,l+1.2,px,sy*.9,PZ-.4,.55,"darkmetal")}
n.push(P_(BOXM(2.4,2.6,1.2,.3),px,0,PZ-1.3,"gunmetal")),n.push(P_(CYL(.7,2.8,12),px,-1.4,PZ-.2,"steel",{tx:Math.PI/2}));
// rocking assembly (beam, horsehead, equaliser, pitman arms)
n.push(P_(BOXM(27,1.6,2,.3),px-2.5,0,PZ,Y,{a:rk})),n.push(P_(BOXM(27,1.8,.3,.05),px-2.5,0,PZ+2,"darkmetal",{a:rk}));
n.push(P_(BOXM(3.2,2.2,7.4,.5),px+12.2,0,PZ-4.4,Y,{a:rk})),n.push(P_(CYL(3.7,2.2,20),px+11.4,1.1,PZ-.6,Y,{tx:Math.PI/2,sz:.55,a:rk}));
n.push(P_(BOXM(1.6,5,1.2,.2),px-15.4,0,PZ-.3,"darkmetal",{a:rk}));
for(const sy of[-2.3,2.3])n.push(P_(BOXM(.7,.5,8.4,.1),px-15.4,sy,PZ-8.3,"steel",{a:rk}));
// polish rod + bridle stroke with the horsehead
const rod={bob:W,ba:3.6,rph:0};n.push(P_(CYL(.22,12,8),px+14,0,l+2.2,"steel",{a:rod})),n.push(P_(BOXM(1.8,1,.5,.1),px+14,0,l+13.5,"darkmetal",{a:rod}));
for(const sy of[-.45,.45])n.push(P_(BOXM(.12,.12,5,.02),px+14,sy,l+13.9,"black",{a:rod}));
n.push(P_(CYL(1.3,2.2,12),px+14,0,l,"darkmetal")),n.push(P_(CYL(.8,1.2,10),px+14,0,l+2.2,"gunmetal")),n.push(P_(BOXM(2.6,.5,.5,.1),px+14,1.3,l+1.1,"red"));
// gearbox, crank + counterweights (turning), motor with belt guard
n.push(P_(BOXM(4.4,4,5.4,.5),cx,0,l+1.2,"armor3")),n.push(P_(CYL(.6,6.4,12),cx,-3.2,cz,"steel",{tx:Math.PI/2}));
for(const sy of[-2.6,2.6])n.push(P_(BOXM(8.6,.7,1.3,.2),cx,sy,cz-.65,"darkmetal",{a:cr})),n.push(P_(BOXM(3.4,1,4.4,.4),cx-4.2,sy,cz-2.2,"rust",{a:cr}));
n.push(P_(BOXM(5,3.4,3.2,.4),cx-5.5,0,l+1.2,"darkmetal")),n.push(P_(BOXM(4,.6,4,.3),cx-3,2.2,l+2,"#c9a227"));
// storage tank, shed and barrels as before
n.push(P_(CYL(9,14,18),14,-12,l,"#b6ae9c")),n.push(P_(DOME(9,2.4,18),14,-12,l+14,"#9c9484")),n.push(P_(BOXM(.6,18.2,.5,.1),14,-12,l+7,"red",{r:.8}));
for(let k=0;k<4;k++)n.push(P_(BOXM(.4,.4,14,.1),14+9*Math.cos(k*1.57),-12+9*Math.sin(k*1.57),l,"darkmetal"));
pipeRun(n,px+14,1,14,-3,l+1.4,.7,"steel");n.push(P_(BOXM(8,5,6,.4),-16,14,l,"concrete")),n.push(P_(WEDGE(8,5,.3,2.4),-16,14,l+6,"rust")),n.push(P_(BOXM(1.6,.4,2.4,.1),-16,16.6,l+2,"glassdark"));for(let k=0;k<3;k++)barrel(n,-4+3*k,16,l);return n}
function paraHangarModel(r,l){const n=[],hw=.3*r,hd=.38*r;n.push(P_(CYL(hw,hd*2,24),-hd,0,l-.6,"armor2",{ty:Math.PI/2}));for(let k=0;k<7;k++)n.push(P_(CYL(hw+.35,1,24),-hd+1+k*(hd*2-2)/6,0,l-.6,"armor3",{ty:Math.PI/2}));n.push(P_(CYL(hw-.6,.6,24),hd-.2,0,l-.6,"dark2",{ty:Math.PI/2}));n.push(P_(CYL(hw+.2,.8,24),-hd-.6,0,l-.6,"armor3",{ty:Math.PI/2}));for(let k=0;k<3;k++)n.push(P_(BOXM(.6,2.8,2.2,.1),-hd-.9,(k-1)*6,l+3,"glass",{e:1}));n.push(P_(CYL(1.8,14,10),hd-10,0,l+2.4,"white",{ty:Math.PI/2})),n.push(P_(CONE(1.8,.4,3.6,10),hd+4,0,l+2.4,"white",{ty:Math.PI/2})),n.push(P_(BOXM(3,16,.4,.1),hd-3,0,l+3.2,"white")),n.push(P_(BOXM(1.6,.3,2.4,.1),hd+.6,0,l+3.6,"glassdark"));n.push(P_(BOXM(.12*r,.12*r,6,.5),.4*r,.36*r,l,"concrete2")),n.push(P_(BOXM(.13*r,.13*r,1,.2),.4*r,.36*r,l+6,"roof")),n.push(P_(BOXM(.12*r+.2,1.2,2,.1),.4*r,.36*r+.06*r,l+3,"glass",{e:1}));n.push(P_(CYL(.25,14,5),-.42*r,.4*r,l,"darkmetal")),n.push(P_(CONE(1.4,.5,5,8),-.42*r,.4*r,l+13,"lightY",{ty:Math.PI/2,a:{orbit:.8,amp:.6,pvx:-.42*r,pvy:.4*r}}));for(let k=0;k<3;k++){crate(n,.3*r+5*k,-.4*r,l,4,"olive",.3*k);n.push(P_(DOME(1.8,1.4,8),.3*r+5*k,-.4*r,l+4,"white"))}return n}
function empSpireModel(r,l){const n=[];n.push(P_(CYL(.3*r,3,16),0,0,l,"concrete2"));for(let k=0;k<4;k++){const a=k*1.571+.78,x=Math.cos(a)*.26*r,y=Math.sin(a)*.26*r;n.push(P_(BOXM(5,5,7,.4),x,y,l+3,"armor3")),n.push(P_(CYL(1.2,3,8),x,y,l+10,"arcbolt",{e:1})),strut3(n,x,y,l+3,0,0,l+44,.6,"darkmetal")}n.push(P_(CYL(4,40,12),0,0,l+3,"darkmetal"));for(let k=0;k<5;k++)n.push(P_(CYL(7.5-.8*k,1.4,16),0,0,l+10+7*k,"arcbolt",{e:1,a:{bob:1.2+.25*k,ba:.8}}));n.push(P_(CYL(5,3,14),0,0,l+43,"steel")),n.push(P_(DOME(4.6,4,14),0,0,l+46,"crystal",{e:1})),n.push(P_(BOXM(12,.6,.6,.1),0,0,l+48,"arcbolt",{e:1,a:{spin:2.4}})),n.push(P_(BOXM(.6,12,.6,.1),0,0,l+48,"arcbolt",{e:1,a:{spin:2.4}}));return n}
function rogueDenModel(r,l){const n=[];n.push(P_(BOXM(20,14,10,.3),-8,-6,l,"rust")),n.push(P_(WEDGE(21,15,1,4),-8,-6,l+10,"steel"));for(let k=0;k<5;k++)n.push(P_(BOXM(.3,14.2,10,.05),-17+4.5*k,-6,l,"darkmetal"));n.push(P_(BOXM(6,.4,7,.1),-4,1.2,l,"black")),n.push(P_(BOXM(8,.3,3,.1),-12,1.3,l+5,"#b0402a"));n.push(P_(BOXM(12,10,8,.3),12,-10,l,"#6f6555")),n.push(P_(WEDGE(13,11,.4,3),12,-10,l+8,"rust"));n.push(P_(BOXM(16,10,.3,.05),6,8,l+8,"#7a8a5a",{tx:.12}));for(const[x,y]of[[-1,3.5],[13,3.5],[-1,12.5],[13,12.5]])n.push(P_(CYL(.3,8,5),x,y,l,"wood"));ell(n,6,8,l,3,3,.6,"rock3");for(let k=0;k<6;k++){const a=k*1.047;n.push(P_(DOME(.8,.6,6),6+Math.cos(a)*3,8+Math.sin(a)*3,l,"rock2"))}n.push(P_(CONE(1.8,.2,3.6,6),6,8,l+.5,"glow",{e:1,a:{bob:3.4,ba:.35}})),n.push(P_(CONE(1.1,.1,2.6,6),6.6,7.6,l+.5,"lightY",{e:1,a:{bob:4.1,ba:.4}}));for(let k=0;k<4;k++)barrel(n,-16+3*k,10,l);crate(n,16,6,l,5,"wood",.4);n.push(P_(BOXM(12,5.4,4,.8),12,14,l,"#5a2a24")),n.push(P_(BOXM(6,4.8,2.4,.6),13,14,l+4,"#5a2a24")),n.push(P_(BOXM(4,4.8,1.8,.2),10,14,l+4.2,"glassdark"));for(const x of[8,16])for(const y of[11.6,16.4])n.push(P_(CYL(1.4,1.2,10),x,y,l+.6,"rubber",{tx:Math.PI/2}));n.push(P_(CYL(.2,9,5),-17,-12,l+10,"darkmetal")),n.push(P_(BOXM(.2,4,2.4,.05),-17,-10,l+16,"black",{a:{orbit:2,amp:.3,pvx:-17,pvy:-12}}));return n}
const CIVX:Record<string,any>={civ7:workshopModel,civ9:terraceModel,bridgehut:bridgeHutModel,oilDerek:oilDerrickModel,paradropHangar:paraHangarModel,empTower:empSpireModel,rogueDen:rogueDenModel};
...
function refineryCore(n,t,r,l,rb){const A="vanguard"===t,O="legion"===t;
if(!A&&!O){
...
if(rb>=3){ell(n,-34,18,l,10,10,24,"bile2");ell(n,-34,18,l+6,8,8,19,"bile",{e:1,a:{bob:1.3,ba:.5}});ell(n,-28,38,l,7,7,18,"bile2");ell(n,-28,38,l+5,5.6,5.6,14,"bile",{e:1,a:{bob:1.6,ba:.5}});
for(const sx of[-1,1])taper3(n,sx*15,32,l,sx*5,38,l+43,3,1.6,"bone")}
if(rb>=4){taper3(n,0,30,l+44,0,56,l+41,4,2.2,"flesh");ell(n,0,56,l+37.5,4,4,5,"carapace2");n.push(P_(DOME(2.6,1.4,10),0,56,l+36.4,"psi",{e:1,tx:Math.PI}));
taper3(n,0,34,l+44,22,-12,l+40,2.6,2.2,"flesh");
depositBay(n,t)}
return}
const hall=A?"armor":"concrete",tow=A?"white":"concrete2",tank=A?"steel":"rust",arm=A?"steel":"rust",gl=A?"glass":"glassdark",lit=A?"crystal":"glow";
if(rb>=1){tier(n,54,42,rb>=2?26:13,-12,-14,l,hall,"rfH"+t+(rb>=2?1:0),.96)}
if(rb>=2){for(let k=0;k<5;k++)n.push(P_(BOXM(6.4,.6,3.2,.1),-32+k*10,7.3,l+13,gl,{e:1}));for(let k=0;k<3;k++)n.push(P_(BOXM(.6,6,3,.1),-39.4,-26+k*10,l+13,"tibG",{e:1}));
n.push(P_(BOXM(18,18,rb>=3?52:26,.6),24,-20,l,tow));
if(A){vent(n,-26,-22,l+26,8,10,"steel");radarSpin(n,-4,-26,l+26,6,.8)}else{for(let k=0;k<3;k++)n.push(P_(BOXM(50,2.2,3.4,.3),-12,-30+k*10,l+26,"rust",{tx:-.35}));stack(n,-33,-30,l+26,2.2,24,"rust");stack(n,-25,-31,l+26,1.8,20,"rust")}}
if(rb>=3){if(A){n.push(P_(BOXM(18.6,18.6,3,.3),24,-20,l+38,"glass",{e:1}));n.push(P_(BOXM(20,20,2,.4),24,-20,l+52,"armor3"));n.push(P_(DOME(2.4,2,10),24,-20,l+54,lit,{e:1}))}
else{for(const z of[16,34])n.push(P_(BOXM(19,19,2.2,.3),24,-20,l+z,"rust"));n.push(P_(BOXM(22,14,8,.5),24,-20,l+52,"rust"));n.push(P_(BOXM(12,.5,2.2,.1),24,-27.3,l+55,lit,{e:1}))}
silo(n,-34,18,l,9,34,tank);silo(n,-28,38,l,7,26,tank);pipeRun(n,-34,12,-34,6,l+9,1.3,tank);pipeRun(n,-28,31,-31,26,l+6,1,tank);
for(const sx of[-1,1])n.push(P_(BOXM(3.4,3.4,46,.4),sx*14,34,l,arm));
n.push(P_(BOXM(34,4.4,4,.4),0,34,l+44,arm))}
if(rb>=4){n.push(P_(BOXM(7,24,3.4,.4),0,46,l+44.6,arm));n.push(P_(CONE(2.2,6.2,7.4,12),0,55,l+37.4,arm));n.push(P_(CYL(1.6,.8,10),0,55,l+36.6,"tibG",{e:1}));spinBeacon(n,0,57,l+48,"lightY");
...
depositBay(n,t)}}
// Ore deposit bay on the dock tile south of the refinery, where harvesters
// park under the unloading arm: a clean flat pad with a glowing ore-coloured
// outline (absolute z so it sits on the ground, not on the foundation).
function depositBay(n,t){const A="vanguard"===t,O="legion"===t,y=64,w=40,d=32,z=3.5;
if(!A&&!O){ell(n,0,y,z-.4,w*.55,d*.55,.9,"carapace2");ell(n,0,y,z,w*.46,d*.46,.55,"flesh");for(let k=0;k<12;k++){const g=k*.5236;n.push(P_(DOME(1,.6,6),Math.cos(g)*w*.5,y+Math.sin(g)*d*.5,z+.2,"bile",{e:1}))}return}
n.push(P_(SLAB(roundRectProfile(w,d,3,3),.7,.3,"dpb"+w),0,y,z-.2,A?"concrete":"concrete2"));
for(const[x,yy,ww,dd]of[[0,y-d/2+1.4,w-4,.7],[0,y+d/2-1.4,w-4,.7],[-w/2+1.4,y,.7,d-4],[w/2-1.4,y,.7,d-4]])n.push(P_(BOXM(ww,dd,.25,.05),x,yy,z+.5,"tibG",{e:1}))}
...
function refineryLife(n,t,l){const A="vanguard"===t,O="legion"===t,PI=Math.PI;
...
for(const[x,y,h]of[[-32,-32,30]]){taper3(n,x,y,l,x+1,y-1,l+h,4.2,1.4,"carapace2");for(let k=0;k<3;k++)ell(n,x,y,l+5+k*h*.25,4-k*.8,4-k*.8,1.6,"carapace");n.push(P_(DOME(1.8,2.2,8),x+1,y-1,l+h-.6,"psi",{e:1,a:{bob:2.2,ba:.5}}))}
...
for(let k=0;k<6;k++)n.push(P_(BOXM(2.6,2.2,24,.3),-36+k*9.6,7.8,l,"concrete2"));
...
for(const x of[-18]){n.push(P_(CYL(4,1.6,14),x,-26,l+26,"darkmetal"));for(let k=0;k<2;k++)n.push(P_(BOXM(7,1,.4,.1),x,-26,l+27.4,"white",{r:k*1.571,a:{spin:7}}))}
...
const FOUNDED:Record<string,1>={power:1,refinery:1,silo:1};
const foundH=(e,t)=>FOUNDED[e]?("syndicate"===t?1.4:"legion"===t?2.4:2):0;
function bldFoundation(n,e,t,r){const H=foundH(e,t);if(!H)return;const W=.94*r,z0=2.2,top=3.6+H,A="vanguard"===t,O="legion"===t,k=r+"_"+H;
if(!A&&!O){ell(n,0,0,z0,.53*W,.53*W,.9,"flesh");ell(n,0,0,z0,.5*W,.5*W,top-z0+.9,"carapace2");for(let q=0;q<9;q++){const g=q*.698+.3,c=Math.cos(g),si=Math.sin(g);taper3(n,c*.3*W,si*.3*W,top-.2,c*.56*W,si*.56*W,z0+.6,1.3,.35,"flesh");q%3||n.push(P_(DOME(1.4,1,8),c*.44*W,si*.44*W,top-.9,"psi",{e:1,a:{bob:1.2+.2*q,ba:.3}}))}return}
n.push(P_(SLAB(roundRectProfile(W+5,W+5,5,3),1.8,.8,"fdL"+k),0,0,z0,O?"darkmetal":"concrete2"));
n.push(P_(SLAB(roundRectProfile(W,W,3,3),top-z0,.5,"fdU"+k),0,0,z0,O?"concrete2":"concrete"));
const h=W/2;
if(A){for(const[sx,sy,w,d]of[[0,-h,W-6,.5],[0,h,W-6,.5],[-h,0,.5,W-6],[h,0,.5,W-6]])n.push(P_(BOXM(w,d,.35,.1),sx,sy,top-.45,"white"));
for(const sx of[-1,1])for(const sy of[-1,1])n.push(P_(BOXM(6,.4,.45,.1),sx*(h-6),sy*(h+2.3),z0+1,"arcbolt",{e:1}));
n.push(P_(BOXM(18,4.5,top-z0-.9,.2),0,h+1.6,z0,"concrete2"));n.push(P_(BOXM(18,2.4,top-z0-.3,.2),0,h+.5,z0,"concrete"))}
else{for(const[sx,sy,w,d]of[[0,-h-.2,W+.4,.7],[0,h+.2,W+.4,.7],[-h-.2,0,.7,W+.4],[h+.2,0,.7,W+.4]])n.push(P_(BOXM(w,d,.9,.1),sx,sy,z0+1.6,"darkmetal"));
bolts(n,0,h+.7,z0+2.05,W-8,0,Math.max(4,W/9|0),"rust");for(const sx of[-1,1])for(const sy of[-1,1])n.push(P_(BOXM(3.4,3.4,top-z0+.5,.3),sx*(h-1.4),sy*(h-1.4),z0,"rust"));}}
let _bFac:any=null;function BMODEL_(e,t,dep,cn,rot,prod,doorT=0){cn=cn||0;_bFac=t;const r=32*BLD[e].size,n=[],a="vanguard"===t,o="legion"===t,d=!1!==dep;const s=.3*r,i=.24*r,l=3.6+foundH(e,t),c=l+s;bldFoundation(n,e,t,r);
...
if("orerig"===e)return oreRigModel(t||"vanguard",l);if(CIVX[e])return CIVX[e](r,l,t);switch(e){case"triturret":{const fac=a?"vanguard":o?"legion":"syndicate",mat=a?"armor":o?"concrete2":"carapace",accent=a?"armor3":o?"rust":"carapace2",plw=.92*r,pld=.62*r,ph=.14*r,rs=16,taper=.92;
...
if("legion"===fac)pipeRun(n,-.3*r,-.08*r,.3*r,-.08*r,bz,.028*r,"rust"),pipeRun(n,-.3*r,-.08*r,0,.14*r,bz,.028*r,"rust"),pipeRun(n,.3*r,-.08*r,0,.14*r,bz,.028*r,"rust");
else if("syndicate"===fac)veins(n,0,.02*r,bz,.34*r,.09*r,10,"psi");
...
break}case"oilDerek":{silo(n,0,.16*r,l,.32*r,.4*r,"rust"),n.push(P_(CYL(.05*r,1.5*r,8),-.22*r,-.18*r,l,"darkmetal")),n.push(P_(BOXM(.46*r,.1*r,.1*r,.3),-.22*r,-.18*r,l+1.2*r,"darkmetal",{r:.4})),pipeRun(n,0,.16*r,-.22*r,-.18*r,l,.06*r,"rust");break}case"paradropHangar":{n.push(P_(BOXM(.9*r,.6*r,.42*r,.15),0,0,l,"concrete2")),n.push(P_(CONE(.52*r,.05*r,.3*r,4),0,0,l+.42*r,"armor3",{r:PI2/8})),n.push(P_(CYL(.025*r,.5*r,6),.33*r,.18*r,l+.42*r,"darkmetal"));break}case"support":{if(a){tier(n,.74*r,.74*r,.36*s,0,0,l,"armor","spv",.93),windows(n,.7*r,.7*r,l+.1*s,.16*s,3);const z=l+.36*s;n.push(P_(CYL(.1*r,.66*r,14),0,0,z,"armor3"));for(let k=0;k<3;k++)n.push(P_(CYL(.27*r-.05*r*k,.035*r,22),0,0,z+.16*r+.18*r*k,k%2?"steel":"armor2",{a:{spin:(k%2?-1:1)*.8}})),n.push(P_(CYL(.2*r-.05*r*k,.02*r,20),0,0,z+.19*r+.18*r*k,"crystal",{e:1}));n.push(P_(DOME(.09*r,.14*r,14),0,0,z+.66*r,"crystal",{e:1}));for(const q of[[-1,-1],[1,-1],[1,1],[-1,1]])n.push(P_(CYL(1.5,.2*r,8),q[0]*r*.27,q[1]*r*.27,z,"steel")),n.push(P_(DOME(2.4,2.4,8),q[0]*r*.27,q[1]*r*.27,z+.2*r,"crystal",{e:1}))}else if(o){tier(n,.88*r,.78*r,.3*s,0,0,l,"concrete","spl",.95),railPosts(n,.84*r,.74*r,l+.3*s,2.6);const z=l+.3*s;n.push(P_(CYL(.25*r,.08*r,22),0,0,z,"darkmetal")),n.push(P_(DOME(.2*r,.2*r,18),0,0,z+.08*r,"red",{e:1}));for(let k=0;k<6;k++){const t=1.047*k;n.push(P_(BOXM(.03*r,.1*r,.06*r,.2),Math.cos(t)*r*.22,Math.sin(t)*r*.22,z+.06*r,"darkmetal",{r:t}))}for(const e of[-1,1]){n.push(P_(BOXM(.1*r,.15*r,.64*r,.6),e*r*.34,0,z,"rust"));for(let k=0;k<4;k++)n.push(P_(BOXM(.13*r,.18*r,.025*r,.2),e*r*.34,0,z+.14*r+.13*r*k,"steel"));n.push(P_(BOXM(.16*r,.2*r,.05*r,.3),e*r*.34,0,z+.64*r,"darkmetal")),n.push(P_(CYL(1.6,1.6,8),e*r*.34,0,z+.69*r,"red",{e:1}))}n.push(P_(BOXM(.7*r,.05*r,.05*r,.4),0,0,z+.58*r,"darkmetal")),stack(n,.3*-r,.3*r,z,3,.16*r,"rust")}else{hive(n,{x:0,y:-.14*r,z:l,r:.22*r,h:.5*s,crown:.1*r,seg:7,crest:!1,vein:"psi"});for(let k=0;k<3;k++){const x=(k-1)*r*.27,y=.2*r;n.push(P_(CYL(.1*r,.05*r,14),x,y,l,"carapace2")),n.push(P_(CYL(.08*r,.22*r,14),x,y,l+.05*r,"psi",{e:1})),n.push(P_(CYL(.083*r,.12*r,14),x,y,l+.27*r,"glassdark"));for(const hz of[.05,.16,.27,.38])n.push(P_(CYL(.092*r,.018*r,14),x,y,l+hz*r,"darkmetal"));n.push(P_(DOME(.095*r,.05*r,14),x,y,l+.39*r,"carapace")),n.push(P_(BOXM(.03*r,.16*r,.03*r,.1),x*.6,y-.12*r,l+.3*r,"carapace2"))}veins(n,0,0,l+.04*r,.42*r,.08*r,7,"psi")}break}case"empTower":{n.push(P_(CYL(.22*r,.7*r,16),0,0,l,"darkmetal"));for(let e=0;e<3;e++)n.push(P_(CYL(.28*r,.06*r,16),0,0,l+.2*r+e*.22*r,"steel"));n.push(P_(DOME(.24*r,.18*r,10),0,0,l+.9*r,"crystal",{e:1}));break}case"rogueDen":{crate(n,-.22*r,-.18*r,l,.34*r,"olive",.3),crate(n,.2*r,-.1*r,l,.28*r,"rust",-.5),n.push(P_(CONE(.3*r,.02*r,.34*r,10),.05*r,.2*r,l,"sand")),n.push(P_(CYL(.03*r,.5*r,6),.28*r,.24*r,l,"darkmetal"));break}case"conyard":{
...
if(a){
// Solar Array: battery/inverter house between two rows of tilted panel racks
if(riseBucket>=1){n.push(P_(BOXM(20,13,8,.4),0,0,l,"armor"));n.push(P_(BOXM(21,14,.8,.2),0,0,l+8,"white"));}
if(riseBucket>=2){for(const sx of[-5.5,0,5.5])n.push(P_(BOXM(3.6,.5,2.4,.1),sx,6.6,l+3.4,"glass",{e:1}));acUnit(n,-5,-2,l+8.8,5,4,0);n.push(P_(CYL(.3,7,6),6,-3,l+8.8,"darkmetal"));n.push(P_(DOME(.8,.8,8),6,-3,l+15.8,"arcbolt",{e:1}));}
if(riseBucket>=3){for(const py of[-19,19])for(const px of[-19,0,19]){for(const sx of[-4,4])n.push(P_(CYL(.35,2.6,6),px+sx,py,l,"steel"));n.push(P_(BOXM(13.4,8.4,.35,.1),px,py,l+2.6,"darkmetal",{tx:-.42}));n.push(P_(BOXM(12.6,7.6,.3,.05),px,py+.1,l+2.85,"crystal",{e:1,tx:-.42}));for(const gx of[-3.1,0,3.1])n.push(P_(BOXM(.18,7.7,.34,.02),px+gx,py+.1,l+2.9,"white",{tx:-.42}))}
for(const py of[-13,13])n.push(P_(BOXM(50,1.2,.6,.1),0,py,l,"darkmetal"));}
if(riseBucket>=4){vawt(n,-27,0,l,15);vawt(n,27,0,l,15);n.push(P_(BOXM(6,5,5,.5),-15,1,l,"darkmetal"));for(const k of[-1.6,0,1.6])n.push(P_(BOXM(6.4,.4,4.4,.05),-15,1+k,l+.3,"steel"));}
}else if(o){
if(riseBucket>=1){
...
break;}
case"refinery":{refineryModel(n,t,r,l,riseBucket);break;}
case"hive":{
const hl=1.02*r,hw=.46*r,hh=.3*s;
n.push(P_(SLAB(roundRectProfile(.58*r,.42*r,3,3),1,.4,"hvpad"),0,0,l,"asphalt"));
const hprof=[[-.46,.24],[-.3,.62],[-.1,.9],[.12,.92],[.3,.66],[.44,.34]];
hprof.forEach(([fx,fr],i)=>{const x=fx*hl,ry=fr*hw,rz=fr*hh*1.55;ell(n,x,0,l+.4,.13*hl,ry*1.06,rz*.8,"flesh");ell(n,x,0,l+.8,.12*hl,ry,rz,i%2?"carapace":"carapace2");i>0&&i<hprof.length-1&&n.push(P_(CONE(.05*r,.012*r,.14*r,6),x,0,l+.8+rz-.02*r,"bone",{ty:-.3}))});
for(const sy of[-1,1])for(let k=0;k<4;k++)n.push(P_(BOXM(.16*hl,.3,.35,.1),(-.3+.2*k)*hl,sy*(.72*hw+.3*(k%2)),l+.6*hh,"psi",{e:1,r:sy*(k%2?.25:-.25)}));
const mx=.5*hl;ell(n,mx,0,l+.6,.07*r,.17*r,.15*r,"flesh");ell(n,mx+.02*r,0,l+1.2,.04*r,.11*r,.1*r,"black");n.push(P_(DOME(.05*r,.05*r,10),mx+.01*r,0,l+.04*r,"psi",{e:1,a:{bob:1.8,ba:.4}}));for(let k=0;k<7;k++){const g=-1.3+k*.43;n.push(P_(CONE(.012*r,.002*r,.05*r,5),mx+.05*r,Math.sin(g)*.13*r,l+1+(.13*r)*(.55+.45*Math.cos(g)),"bone",{ty:PI2+.4*Math.cos(g),r:-.5*Math.sin(g)}))}
for(const[cx,cy]of[[-.12*hl,.18*hw],[.12*hl,-.2*hw]])for(let k=0;k<4;k++){const g=k*1.57+.4;n.push(P_(DOME(.028*r,.034*r,9),cx+Math.cos(g)*.03*r,cy+Math.sin(g)*.03*r,l+.8+.9*hh*1.55-.01*r,"bile",{e:1,a:{bob:1.1+.3*k,ba:.5}}))}
for(const sy of[-1,1]){const x0=-.26*hl,y0=sy*.26*hw,z0=l+.8+.5*hh*1.55;taper3(n,x0,y0,z0,x0-.02*r,y0+sy*.02*r,z0+.16*r,.035*r,.022*r,"flesh");n.push(P_(CONE(.03*r,.034*r,.02*r,8),x0-.02*r,y0+sy*.02*r,z0+.16*r,"carapace2"));n.push(P_(DOME(.018*r,.014*r,8),x0-.02*r,y0+sy*.02*r,z0+.175*r,"psi",{e:1}))}
for(const side of[-1,1])for(let i=0;i<2;i++){const x=(-.28+.5*i)*hl,y=side*.6*hw;ell(n,x,y,l+.4,.06*r,.05*r,.07*r,"flesh");n.push(P_(DOME(.022*r,.018*r,8),x,y+side*.045*r,l+.05*r,"psi",{e:1}))}
veins(n,.02*hl,0,l+.55*hh,.16*hl,.22*hh,8,"psi");
if(d){ell(n,-.7*r,0,l+.2,.2*r,.1*r,.04*r,"flesh");for(const sy of[-1,1])taper3(n,-.9*r,sy*.1*r,l+.4,-.5*r,sy*.1*r,l+.6,.012*r,.012*r,"bone");for(let k=0;k<4;k++)n.push(P_(BOXM(.012*r,.2*r,.01*r,.005),(-.84+.1*k)*r,0,l+.04*r,"psi",{e:1}));
for(const sx of[-1,1])n.push(P_(DOME(.11*r,.1*r,12),sx*.2*hl,0,l+.8+.8*hh*1.55,"carapace"));
n.push(P_(DOME(.09*r,.08*r,10),0,0,l+.8+.95*hh*1.55,"psi",{e:1,a:{spin:.6}}));
}else{
for(const sx of[-1,1])n.push(P_(DOME(.12*r,.1*r,10),sx*.18*hl,0,l+.8+.8*hh*1.55,"carapace2"));
}
break;}
case"barracks":{
...
// (as armor3-on-armor3 was for Vanguard) makes the whole shutter assembly
...
// plain tapered wall instead and Syndicate keeps tier()'s flat cap + railing.
if(o||a){
...
// Legion's WEDGE already runs ridgeH->eaveH with eaveH>0, and Vanguard's
...
if(riseBucket>=4){n.push(P_(DOME(.26*r,.2*r,20),0,0,c+3.2+.3*r,"armor3"));for(let e=0;e<4;e++){const t=e*PI2+.78;n.push(P_(CYL(1.9,1.3,10),Math.cos(t)*r*.24,Math.sin(t)*r*.24,c+3.2+.46*r,"arcbolt",{e:1}))}}
}else{
...
n.push(P_(SLAB(roundRectProfile(pw,pd,4,3),1.65,.45,"afp"+pi),px,py,2.35,"asphalt"));
if(riseBucket>=1){
...
// Offshore yard: built out on open water. Piled deck at the back, two finger
// piers framing an open berth (facing +y, where ships launch), and a hull on
// the slip. Footing sits at the waterline, so pilings run down out of sight.
const DZ=l-1.4,PIL=a?"steel":o?"rust":"carapace2",DECK=a?"concrete2":o?"concrete":"carapace2";
n.push(P_(BOXM(.9*r,.3*r,1.8,.4),0,-.3*r,DZ,DECK)),n.push(P_(BOXM(.92*r,.02*r,.6,.1),0,-.15*r,DZ,"black"));
for(const sx of[-1,1])n.push(P_(BOXM(.2*r,.64*r,1.8,.4),sx*.35*r,.1*r,DZ,DECK)),n.push(P_(BOXM(.14*r,.6*r,.12,.05),sx*.35*r,.1*r,DZ+1.8,a?"armor3":o?"concrete2":"carapace"));
for(let k=0;k<5;k++)n.push(P_(CYL(.028*r,12,8),(-.4+.2*k)*r,-.42*r,DZ-11,PIL)),n.push(P_(CYL(.028*r,12,8),(-.4+.2*k)*r,-.18*r,DZ-11,PIL));
for(const sx of[-1,1])for(let k=0;k<4;k++){const y=(-.05+.13*k)*r;n.push(P_(CYL(.026*r,12,8),sx*.43*r,y,DZ-11,PIL)),n.push(P_(CYL(.026*r,12,8),sx*.27*r,y,DZ-11,PIL)),n.push(P_(CYL(1.3,2.2,8),sx*.43*r,y,DZ+1.8,"darkmetal")),n.push(P_(CYL(1.5,3.2,8),sx*.25*r,y+.06*r,DZ,"black"))}
for(const sx of[-1,1])n.push(P_(BOXM(.012*r,.64*r,1.6,.1),sx*.448*r,.1*r,DZ+1.8,"steel"));
if(riseBucket>=3){if(a||o){const hm=a?"armor3":"gunmetal";n.push(P_(BOXM(.26*r,.44*r,.05*r,.3),0,.12*r,DZ-.6,hm));for(let k=0;k<6;k++){const y=(-.07+.075*k)*r;n.push(P_(BOXM(.3*r,.014*r,.12*r,.1),0,y,DZ-.6,"steel"))}n.push(P_(WEDGE(.26*r,.1*r,.08*r,.04*r),0,.38*r,DZ-.6,hm,{r:PI2}))}else{n.push(P_(DOME(.2*r,.03*r,16),0,.12*r,DZ-.9,"psi",{e:1}));n.push(P_(DOME(.1*r,.12*r,12),0,.12*r,DZ-.4,"carapace",{a:{bob:.6}}));veins(n,0,.12*r,DZ,.16*r,.05*r,8,"psi")}}
if(a){
if(riseBucket>=1){n.push(P_(BOXM(.3*r,.2*r,.26*r,1.2),-.24*r,-.3*r,l,"armor")),windows(n,.29*r,.19*r,l+.12*r,.07*r,3,"glass"),n.push(P_(BOXM(.18*r,.14*r,.12*r,.8),-.24*r,-.3*r,l+.26*r,"armor2")),glassBand(n,.19*r,.15*r,l+.32*r,.05*r)}
if(riseBucket>=2){for(const sx of[-1,1])n.push(P_(BOXM(.05*r,.05*r,.5*r,.3),sx*.35*r,.02*r,l,"steel")),n.push(P_(BOXM(.05*r,.05*r,.5*r,.3),sx*.35*r,.22*r,l,"steel"));n.push(P_(BOXM(.76*r,.06*r,.06*r,.3),0,.02*r,l+.5*r,"armor3")),n.push(P_(BOXM(.76*r,.06*r,.06*r,.3),0,.22*r,l+.5*r,"armor3")),n.push(P_(BOXM(.1*r,.26*r,.07*r,.3),.06*r,.12*r,l+.44*r,"gold",{a:{patrol:.07,pw:.5*r,ph:1}})),hazard(n,0,.02*r,l+.56*r,.7*r,0)}
if(riseBucket>=4){dish(n,-.34*r,-.36*r,l+.26*r,.05*r),n.push(P_(CYL(1.2,1.2,8),-.24*r,-.3*r,l+.44*r,"glow",{e:1}))}
}else if(o){
if(riseBucket>=1){n.push(P_(BOXM(.46*r,.22*r,.18*r,1.4),-.18*r,-.3*r,l,"concrete")),n.push(P_(BOXM(.4*r,.18*r,.04*r,1),-.18*r,-.3*r,l+.18*r,"concrete2")),windows(n,.44*r,.21*r,l+.08*r,.05*r,4,"glassdark")}
if(riseBucket>=2){stack(n,-.34*r,-.36*r,l+.18*r,.03*r,.22*r,"rust"),stack(n,-.26*r,-.36*r,l+.18*r,.026*r,.17*r,"rust"),crane(n,.35*r,-.02*r,l,.44*r,.36*r,"gunmetal")}
if(riseBucket>=4){for(const sx of[-1,1])n.push(P_(CYL(1.3,1.3,8),sx*.35*r,.4*r,l+.4,"red",{e:1}));pipeRun(n,-.18*r,-.3*r,-.02*r,-.3*r,l+.04*r,.016*r,"rust")}
}else{
if(riseBucket>=1){n.push(P_(TSLAB(circleProfile(.2*r,9),.3*r,.8,"nyh"),-.22*r,-.3*r,l,"carapace2")),n.push(P_(DOME(.17*r,.15*r,12),-.22*r,-.3*r,l+.3*r,"carapace")),veins(n,-.22*r,-.3*r,l+.1*r,.19*r,.17*r,7,"psi")}
if(riseBucket>=2){bioArm(n,.3*r,-.2*r,l,.42*r,.24*r,"carapace2"),n.push(P_(DOME(.08*r,.07*r,10),.14*r,-.32*r,l,"carapace2"))}
if(riseBucket>=4){n.push(P_(DOME(.05*r,.05*r,10),-.22*r,-.3*r,l+.45*r,"psi",{e:1,a:{spin:.6}}));for(const sx of[-1,1])n.push(P_(CONE(.03*r,.004*r,.16*r,6),sx*.43*r,.42*r,DZ+1.8,"carapace",{ty:.3*sx}))}
}
break;}case"civ6":for(const p of marketRowModel(r))n.push(p);break;case"civ7":{const w=.9*r,d=.7*r,h=.34*r;n.push(P_(BOXM(w,d,h,.4),0,0,l,"concrete2")),n.push(P_(BOXM(w*.96,d*.5,.06*r,.2),0,-d*.2,l+h,"darkmetal")),n.push(P_(CYL(.05*r,.36*r,8),.32*r,.2*r,l+h,"rust")),n.push(P_(BOXM(.3*r,.04*r,.2*r,.1),-.2*r,-d/2-.01*r,l+.06*r,"darkmetal")),n.push(P_(BOXM(.3*r,.04*r,.2*r,.1),.05*r,-d/2-.01*r,l+.06*r,"darkmetal"));break}case"civ8":for(const p of tavernModel(r))n.push(p);break;case"civ9":{const w=.86*r;for(let tier=0;tier<3;tier++){const tw=w*(1-.22*tier),th=.32*r,tz=l+tier*th;n.push(P_(BOXM(tw,.82*tw,th,.6),0,-.06*r*tier,tz,tier%2?"concrete2":"neutral")),windows(n,tw*.85,.7*tw,tz+.5*th,.09*r,2+tier%2,"glassdark")}break}case"bridgehut":tier(n,.7*r,.66*r,.36*r,0,0,l,"neutral","cv2",.94),n.push(P_(SLAB(roundRectProfile(.78*r,.74*r,3,3),2.4,.7,"cv2r"),0,0,l+.36*r,"rust")),windows(n,.56*r,.5*r,l+.12*r,.09*r,1,"glassdark"),n.push(P_(CYL(1.4,.14*r,10),.2*r,-.18*r,l+.36*r+2.4,"darkmetal"));break;case"civ4":for(const p of churchModel())n.push(p);break;case"civ5":for(const p of tenementModel(r))n.push(p);break;case"civ3":for(const p of highRiseModel(r))n.push(p)}return bldExtras(n,e,t,r,l,s,riseBucket,cn),detailPass(n,e,t,r),n}function BTURRET_(e,t,r){if("cannon"===e){const c=[];c.push(P_(CYL(10,2.4,18),0,0,9.2,"darkmetal"));boltRing(c,0,0,11.6,8.6,12,"steel");c.push(P_(BOXM(11,15,7,.8),1,0,11.6,"armor2"));c.push(P_(WEDGE(4,15,7,3),8,0,11.6,"armor2"));c.push(P_(CYL(1.8,24,12),6,0,15.4,"gunmetal",{ty:PI2}));c.push(P_(CYL(2.5,3.2,12),29,0,15.4,"darkmetal",{ty:PI2}));for(const sy of[-2.6,2.6])c.push(P_(CYL(.8,8,8),4,sy,17.6,"steel",{ty:PI2}));c.push(P_(BOXM(4,5,3,.3),-5,4.5,11.6,"olive"));c.push(P_(BOXM(3,3,4,.3),-5,-4.5,11.6,"darkmetal"));return c}const n="vanguard"===t,a="legion"===t;if("def1"===e){const e=[];return n?(e.push(P_(CYL(5.6,4.5,12),0,0,14.6,"armor")),e.push(P_(SLAB(hexProfile(11,9),5,1.6,"d1vt"),0,0,19.1,"body")),e.push(P_(CYL(1.5,15,9),5,0,21.6,"gunmetal",{ty:PI2})),e.push(P_(CYL(2,3.5,9),17,0,21.6,"steel",{ty:PI2}))):a?(e.push(P_(CYL(6.4,5,10),0,0,11.6,"armor2")),e.push(P_(SLAB(roundRectProfile(12,10,2,2),5.5,1.5,"d1lt"),0,0,16.6,"body")),e.push(P_(CYL(1.8,13,8),5,-2.2,19.1,"gunmetal",{ty:PI2})),e.push(P_(CYL(1.8,13,8),5,2.2,19.1,"gunmetal",{ty:PI2}))):(e.push(P_(DOME(6.5,5,12),0,0,9.6,"carapace")),e.push(P_(CONE(4,1.6,9,9),4,0,12.6,"body",{ty:PI2})),e.push(P_(CYL(1.2,4,7),13,0,12.6,"psi",{ty:PI2,e:1}))),e}if("aa"===e){const e=[],t=n?15.6:a?14.6:13.6;if(e.push(P_(CYL(5.2,4,12),0,0,t-4,"armor3")),n){e.push(P_(SLAB(hexProfile(10,9),4.5,1.4,"aavt"),0,0,t,"body"));for(const r of[-3.2,3.2])e.push(P_(BOXM(11,3.6,3.6,1),4,r,t+3.4,"steel")),e.push(P_(CONE(1.4,.4,2.6,7),10,r,t+3.4,"red",{ty:PI2}))}else if(a){e.push(P_(SLAB(roundRectProfile(11,10,2,2),5,1.4,"aalt"),0,0,t,"body"));for(const r of[-2.6,2.6])e.push(P_(CYL(1.5,14,8),4,r,t+4,"gunmetal",{ty:PI2,tx:0}));e.push(P_(BOXM(5,7,3,1),-4,0,t+4,"darkmetal"))}else{e.push(P_(CONE(5.5,2.5,7,10),0,0,t,"carapace"));for(let r=0;r<3;r++){const n=2.09*r;e.push(P_(CYL(.9,12,6),2,2.4*Math.cos(n),t+4+2.4*Math.sin(n),"psi",{ty:PI2,e:1}))}}return e}if("def2"===e&&"syndicate"===t){const e=[P_(DOME(5.5,4.5,12),0,0,31.6,"psi",{e:1})];for(let t=0;t<3;t++){const n=2.09*t+(r?.5:0);e.push(P_(CYL(1,10,6),4*Math.cos(n),4*Math.sin(n),34,"crystal",{e:1}))}return e}return null}const VEH_ACCENT:Record<string,number[]>={hover:[30,20,13.4,0],ravager:[34,17,10.6,1],longbow:[40,16,9.8,0],harvester:[0,0,0,0]};const ACC_FAC=["vanguard","legion","syndicate"];
...
function UMODEL(e,t,extra){const fac=extra&&"object"==typeof extra?extra.fac:void 0,_a=getAssetModel(e,fac);if(_a)return _a;const um=vehAccent(UMODEL_(e,t||0,extra),e,extra);return UNITS[e]&&(UNITS[e].fly||NO_ATTACH[e])?um:attachFloating(um,"syndicate"===fac?"carapace2":"darkmetal")}const NO_ATTACH:Record<string,1>={drone:1,tickdrone:1,skyjack:1,hivetrans:1};function UTURRET(e,t,extra?){return UTURRET_(e,t||0,extra)}const YREMAP:Record<string,string>={steel:"bone",metal:"carapace",darkmetal:"carapace2",gunmetal:"carapace2",concrete:"carapace",concrete2:"carapace2",rust:"flesh",armor:"carapace",armor2:"carapace",armor3:"carapace2",lightY:"psi",glass:"psi",glassdark:"carapace2",roof:"carapace2",tread:"carapace2",rubber:"flesh",wood:"bone",olive:"carapace2",red:"flesh",arcbolt:"psi",glow:"psi",crystal:"psi",white:"bone",nose:"carapace",gold:"bile"};const YDRESS_SKIP:Record<string,number>={wall:1,gate:1};
const _mbox=new WeakMap;function meshBox(m){let b=_mbox.get(m);if(b)return b;b=[1e9,1e9,1e9,-1e9,-1e9,-1e9];for(const t of m)for(const p of t.p)for(let i=0;i<3;i++)p[i]<b[i]&&(b[i]=p[i]),p[i]>b[i+3]&&(b[i+3]=p[i]);return _mbox.set(m,b),b}
function heightField(n){const boxes=[];for(const q of n){if(!q||!q.m||q.a||!q.m.length)continue;const b=meshBox(q.m),g=q.sx||1,yy=q.sy||1,zz=q.sz||1,ct=Math.cos(q.tx||0),st=Math.sin(q.tx||0),cy=Math.cos(q.ty||0),sy=Math.sin(q.ty||0),co=Math.cos(q.r||0),so=Math.sin(q.r||0);let x0=1e9,y0=1e9,x1=-1e9,y1=-1e9,z1=-1e9;for(const X of[b[0],b[3]])for(const Y of[b[1],b[4]])for(const Z of[b[2],b[5]]){let P=X*g,k=Y*yy,S=Z*zz;if(q.tx){const e=k*st+S*ct;k=k*ct-S*st,S=e}if(q.ty){const e=-P*sy+S*cy;P=P*cy+S*sy,S=e}const wx=P*co-k*so+(q.x||0),wy=P*so+k*co+(q.y||0),wz=S+(q.z||0);wx<x0&&(x0=wx),wx>x1&&(x1=wx),wy<y0&&(y0=wy),wy>y1&&(y1=wy),wz>z1&&(z1=wz)}(x1-x0)*(y1-y0)>4&&boxes.push([x0,y0,x1,y1,z1])}return(x,y)=>{let h=0;for(const B of boxes)x>=B[0]&&x<=B[2]&&y>=B[1]&&y<=B[3]&&B[4]>h&&(h=B[4]);return h}}
function syndicateDress(n,key){const B=BLD[key];if(!B)return n;const r=32*B.size;const out=n.map(q=>{if(!q)return q;if(YREMAP[q.c])return Object.assign({},q,{c:YREMAP[q.c]});if("string"==typeof q.c&&"#"===q.c[0]&&7===q.c.length){const[R,G,Bc]=rgbOf(q.c),l=(R+G+Bc)/3;return Object.assign({},q,{c:q.e?"psi":R>G+40&&R>Bc+30?"flesh":l<70?"carapace2":l<150?"carapace":"bone"})}return q});if(YDRESS_SKIP[key]||B.size<2)return out;
const H=heightField(n),base=6.5,walls=[];for(let k=0;k<8;k++){const a=k/8*6.283+.39,c=Math.cos(a),sn=Math.sin(a);let wx=0,wy=0,wh=0;for(let d=2;d<=.62*r;d+=1){const h=H(c*d,sn*d);h>base+3&&(wx=c*d,wy=sn*d,wh=h)}wh&&walls.push([wx,wy,wh,a])}
let top=0;for(const q of n)q&&!q.a&&q.m&&(top=Math.max(top,q.z+meshBox(q.m)[5]*(q.sz||1)));
walls.forEach(([wx,wy,wh,a],k)=>{const c=Math.cos(a),sn=Math.sin(a),ox=wx-c*.8,oy=wy-sn*.8,ex=wx+c*.34*r,ey=wy+sn*.34*r;const el=Math.min(.64*r,Math.hypot(ex,ey));const fx=Math.cos(a+.12)*el,fy=Math.sin(a+.12)*el;taper3(out,ox,oy,5.4,fx,fy,3.5,1.8,.35,"flesh");taper3(out,ox,oy,6,fx,fy,3.9,.22,.12,"psi",{e:1});out.push(P_(DOME(2,1.3,8),ox,oy,3.4,"carapace2"));
if(k%2)out.push(P_(CONE(1.5,.1,Math.min(.35*wh,14)+3,7),wx-c*.4,wy-sn*.4,3.4,"bone",{ty:.35,r:a}));else out.push(P_(DOME(2.6,2.2,10),wx+c*.6,wy+sn*.6,2.8,"bile",{e:1,a:{bob:1.3+.23*k,ba:.45}}))});
const cand=[];for(let k=0;k<10;k++){const a=k/10*6.283+.2;for(const rr of[.08,.16,.24]){const x=Math.cos(a)*rr*r,y=Math.sin(a)*rr*r,h=H(x,y);h>base+6&&cand.push([x,y,h])}}cand.sort((p,q)=>q[2]-p[2]);const picks=[];for(const c of cand){if(picks.length>=3)break;picks.every(p=>Math.hypot(p[0]-c[0],p[1]-c[1])>.18*r)&&c[2]<top+1&&picks.push(c)}
if("factory"===key){const hd=.25*r,tp=H(0,0);for(let i=0;i<5;i++){const x=(-.34+.17*i)*r,h0=Math.max(H(x,-hd+1),base);taper3(out,x,-hd-1,h0-1,x,-.5*hd,tp+4,1.2,1,"bone");taper3(out,x,-.5*hd,tp+4,x,.5*hd,tp+5.5,1,1,"bone");taper3(out,x,.5*hd,tp+5.5,x,hd+1,h0-1,1,1.2,"bone")}}
picks.forEach(([x0,y0,h0],k)=>{const g=Math.atan2(y0,x0)||k*2.1,z0=h0-.8,A={orbit:.6+.23*k,amp:.28,pvx:x0,pvy:y0};let px=x0,py=y0,pz=z0;const segs=[[1.5*Math.cos(g),1.5*Math.sin(g),4.4],[2.4*Math.cos(g+.6),2.4*Math.sin(g+.6),3.6],[1.6*Math.cos(g+1.2),1.6*Math.sin(g+1.2),2.6]];out.push(P_(DOME(1.7,1.3,8),x0,y0,z0-.3,"flesh"));segs.forEach(([dx,dy,dz],i)=>{const dh=Math.hypot(dx,dy),L=Math.hypot(dh,dz);out.push(P_(CONE(1.1-.3*i,.8-.3*i,L,8),px,py,pz,i%2?"carapace2":"carapace",{ty:Math.atan2(dh,dz),r:Math.atan2(dy,dx),a:A}));px+=dx,py+=dy,pz+=dz});out.push(P_(DOME(.7,.6,7),px,py,pz-.2,"psi",{e:1,a:A}))});
return out}
const GROUND_DROP=3.8;function groundModel(m){return m.filter(q=>!(q&&"asphalt"===q.c&&(q.z||0)>2.5&&(q.z||0)<6.5)).map(q=>q?Object.assign({},q,{z:(q.z||0)-GROUND_DROP}):q)}
// Model hygiene: finds parts that aren't connected (through touching parts)
// to the ground and fixes them — near misses are nudged into contact, low
// strays settle onto the ground, and anything higher gets a support strut.
// Moving (animated) parts are left alone; they only rest where they are.
function partBox(a){const r=a.r||0,o=Math.cos(r),s=Math.sin(r),i=a.tx||0,l=a.ty||0,c=Math.cos(i),d=Math.sin(i),f=Math.cos(l),h=Math.sin(l),g=a.sx||1,y=a.sy||1,x=a.sz||1,mn=[1e9,1e9,1e9],mx=[-1e9,-1e9,-1e9];for(const t of a.m)for(let b=0;b<3;b++){let P=t.p[b][0]*g,k=t.p[b][1]*y,Z=t.p[b][2]*x;if(i){const e=k*d+Z*c;k=k*c-Z*d,Z=e}if(l){const e=-P*h+Z*f;P=P*f+Z*h,Z=e}const v=[P*o-k*s+(a.x||0),P*s+k*o+(a.y||0),Z+(a.z||0)];for(let q=0;q<3;q++)v[q]<mn[q]&&(mn[q]=v[q]),v[q]>mx[q]&&(mx[q]=v[q])}return{mn,mx}}
function attachFloating(parts,strutMat,depth?){if(!parts||parts.length>3000||(depth||0)>3)return parts;const T=.3,idxs=[],B=[];parts.forEach((p,i)=>{p&&p.m&&p.m.length&&!p.a&&(idxs.push(i),B.push(partBox(p)))});const n=B.length;if(!n)return parts;
const ov=(A,C)=>A.mn[0]<=C.mx[0]+T&&C.mn[0]<=A.mx[0]+T&&A.mn[1]<=C.mx[1]+T&&C.mn[1]<=A.mx[1]+T&&A.mn[2]<=C.mx[2]+T&&C.mn[2]<=A.mx[2]+T;
const out=parts.slice();for(let pass=0;pass<16;pass++){const ok=new Uint8Array(n),st=[];for(let i=0;i<n;i++)B[i].mn[2]<=.9&&(ok[i]=1,st.push(i));while(st.length){const i=st.pop();for(let j=0;j<n;j++)!ok[j]&&ov(B[i],B[j])&&(ok[j]=1,st.push(j))}
let any=!1;const cid=new Int32Array(n).fill(-1),cls=[];for(let i=0;i<n;i++)if(!ok[i]&&cid[i]<0){const c=[i];cid[i]=cls.length;for(let k=0;k<c.length;k++)for(let j=0;j<n;j++)!ok[j]&&cid[j]<0&&ov(B[c[k]],B[j])&&(cid[j]=cls.length,c.push(j));cls.push(c)}
for(const c of cls){any=!0;const mn=[1e9,1e9,1e9],mx=[-1e9,-1e9,-1e9];for(const i of c)for(let q=0;q<3;q++)mn[q]=Math.min(mn[q],B[i].mn[q]),mx[q]=Math.max(mx[q],B[i].mx[q]);
let best=null,bd=1e9,bv=null;for(const i of c)for(let j=0;j<n;j++){if(!ok[j])continue;const v=[0,0,0];let d=0;for(let q=0;q<3;q++){const g1=B[j].mn[q]-B[i].mx[q],g2=B[i].mn[q]-B[j].mx[q];v[q]=g1>0?g1+.05:g2>0?-(g2+.05):0,d+=v[q]*v[q]}d=Math.sqrt(d);d<bd&&(bd=d,best=j,bv=v)}
const move=(dx,dy,dz)=>{for(const i of c){const p=out[idxs[i]]=Object.assign({},out[idxs[i]],{x:(out[idxs[i]].x||0)+dx,y:(out[idxs[i]].y||0)+dy,z:(out[idxs[i]].z||0)+dz});B[i]={mn:[B[i].mn[0]+dx,B[i].mn[1]+dy,B[i].mn[2]+dz],mx:[B[i].mx[0]+dx,B[i].mx[1]+dy,B[i].mx[2]+dz]};void p}};
if(best!==null&&bd<=3)move(bv[0],bv[1],bv[2]);else if(mn[2]<=3.5)move(0,0,-mn[2]+.05);else{let lo=c[0];for(const i of c)B[i].mn[2]<B[lo].mn[2]&&(lo=i);const cx=(B[lo].mn[0]+B[lo].mx[0])/2,cy=(B[lo].mn[1]+B[lo].mx[1])/2,wmin=Math.min(B[lo].mx[0]-B[lo].mn[0],B[lo].mx[1]-B[lo].mn[1]);let top=0;for(let j=0;j<n;j++)ok[j]&&B[j].mn[0]<=cx&&B[j].mx[0]>=cx&&B[j].mn[1]<=cy&&B[j].mx[1]>=cy&&B[j].mx[2]<=mn[2]+.2&&(top=Math.max(top,B[j].mx[2]));const len=B[lo].mn[2]-top+.3,rad=clamp(.3*wmin,.25,1.1),sp=P_(CYL(rad,len,10),cx,cy,top-.15,strutMat||"darkmetal");out.push(sp);const bx=partBox(sp);idxs.push(out.length-1),B.push(bx);for(const i of c)ok[i]=1}}
if(!any)break;if(B.length!==n)return attachFloating(out,strutMat,(depth||0)+1)}return out}
function BMODEL(e,t,d,cn,rot,prod=null,doorT=0){const _a=getAssetModel(e,t);if(_a)return"syndicate"===t?syndicateDress(_a,e):_a;const m=BMODEL_(e,t,d,cn,rot,prod,doorT);const g=groundModel("syndicate"===t?syndicateDress(m,e):m);return"refinery"===e?g:attachFloating(g,"syndicate"===t?"carapace2":"darkmetal")}function BTURRET(e,t,r){const m=BTURRET_(e,t,r||0);return m&&m.length?m.map(q=>q?Object.assign({},q,{z:(q.z||0)-GROUND_DROP}):q):m}
/*
...
Object.assign(window, {crateModel,
  attachFloating, partBox, DOME, BOXM, ell, taper3, lowPoly, SLAB, TSLAB, INF_FAC,
  ...
  subHull, tankHull, turretProfile, tankTurret, harvesterLegion, harvesterVanguard, harvesterSyndicate,
