...
function legWalker(e,t,r,n,a,legCount){legCount=legCount||6;const perSide=legCount/2,hipZ=.8*n,spanX=.7*t,step=perSide>1?spanX/(perSide-1):0,br=16&(a||0)?1:0,L=(p,q,k)=>p+(q-p)*k;for(const side of[-1,1])for(let i=0;i<perSide;i++){const hx=-spanX/2+i*step,hy=side*(.5*r-.8),grp=(i+(side>0?0:1))%2,c=(((a||0)&3)+2*grp)&3,lift=1===c?3:0,sw=[-1,0,1,0][c]*.08*t,fw=(i-(perSide-1)/2)*.9,kx=hx+.4*sw+fw,ky=side*(.5*r+3.6),kz=hipZ+2.4+.6*lift,fx=hx+sw+1.6*fw,fy=side*(.5*r+7.2-.6*lift),fz=.3+.6*lift;
e.push(P_(DOME(2.1,1.7,9),hx,hy,hipZ-.9,"flesh"));e.push(P_(DOME(1.7,1.3,9),hx,hy+side*.4,hipZ-.2,"carapace2"));
taper3(e,hx,hy,hipZ,kx,ky,kz,1.45,1,"carapace");taper3(e,L(hx,kx,.15),L(hy,ky,.15)+side*.2,L(hipZ,kz,.15)+.9,L(hx,kx,.8),L(hy,ky,.8)+side*.2,L(hipZ,kz,.8)+.7,.22,.18,"psi",{e:1});
e.push(P_(DOME(1.25,1.1,8),kx,ky,kz-.5,"flesh"));e.push(P_(CONE(.6,.05,2.4,6),kx,ky,kz+.4,"bone",{ty:-.35*side,r:PI2*side}));e.push(P_(DOME(.45,.4,6),kx,ky+side*.8,kz,"psi",{e:1}));
const mx=L(kx,fx,.55),my=L(ky,fy,.55),mz=L(kz,fz,.55)+.6;taper3(e,kx,ky,kz,mx,my,mz,.95,.7,"carapace2");e.push(P_(DOME(.75,.6,7),mx,my,mz-.3,"flesh"));taper3(e,mx,my,mz,fx,fy,fz+.6,.62,.2,"carapace");taper3(e,fx,fy,fz+.8,fx+.6*sw/(.08*t||1)*.2,fy,fz-.3,.3,.04,"bone")}}
function ell(a,x,y,z,rx,ry,rz,mat,o?){a.push(P_(DOME(1,1,16),x,y,z,mat,Object.assign({sx:rx,sy:ry,sz:rz},o||{})))}
function organicBody(a,e,t,s,n,frame){const br=16&(frame||0)?1:0,bs=1+.07*br,cz=s+.6;
ell(a,0,0,cz,.46*e,.48*t,2.6,"flesh",{tx:Math.PI});
for(let k=0;k<4;k++){const x=(.27-.18*k)*e,rw=(.56-.06*Math.abs(k-1.3))*t,rh=(6.6-.7*k)+.35*br*(k%2);ell(a,x-.03*e,0,cz,.15*e,rw*1.04,rh-1,"carapace");ell(a,x,0,cz+.4,.14*e,rw,rh,"body")}
for(let k=0;k<5;k++)a.push(P_(CONE(.8-.08*k,.06,2.6-.25*k,6),(.26-.14*k)*e,0,cz+6.6-.55*k,"bone",{ty:-.4}));
const hx=.47*e;ell(a,hx,0,cz+.6,.14*e,.3*t,4.2,"carapace2");ell(a,hx+.03*e,0,cz+.4,.12*e,.24*t,1.8,"flesh",{tx:Math.PI});
for(const[ey,ez,er]of[[0,3.6,1],[-.13,3,.7],[.13,3,.7],[-.2,2,.55],[.2,2,.55]])a.push(P_(DOME(er,er*.85,8),hx+.11*e,ey*t,cz+ez,"psi",{e:1}));
for(const sy of[-1,1]){a.push(P_(CONE(.8,.08,4.6,7),hx+.1*e,sy*.12*t,cz+.8,"bone",{ty:PI2+.3,r:-sy*.4}));a.push(P_(CONE(.5,.06,2.8,6),hx+.08*e,sy*.2*t,cz+2,"bone",{ty:PI2-.1,r:-sy*.7}))}
const tx=-.5*e;ell(a,tx,0,cz,.2*e*bs,.3*t*bs,4.6*bs,"flesh");ell(a,tx-.02*e,0,cz+.6,.17*e,.26*t,4.4,"carapace2",{});for(const sy of[-1,1])a.push(P_(DOME(1.7*bs,1.5*bs,9),tx-.08*e,sy*.14*t,cz+3.4,"bile",{e:1}));a.push(P_(DOME(1.3*bs,1.2*bs,9),tx-.18*e,0,cz+2.6,"bile",{e:1}));
for(const sy of[-1,1])for(let k=0;k<3;k++)a.push(P_(BOXM(.14*e,.28,.3,.1),(.2-.17*k)*e,sy*(.5*t+.2*(k%2)),cz+2.2+.5*k,"psi",{e:1,r:sy*(k%2?.3:-.3)}));
a.push(P_(DOME(1.6*bs,1.3*bs,9),.05*e,-.4*t,cz+4.2,"bile",{e:1}));a.push(P_(DOME(1.3*bs,1.1*bs,9),-.14*e,.42*t,cz+4.4,"bile",{e:1}));
return a}
